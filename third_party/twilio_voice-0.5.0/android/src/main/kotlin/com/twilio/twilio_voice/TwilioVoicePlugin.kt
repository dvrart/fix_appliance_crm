...
class TwilioVoicePlugin : FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler,
    ...
    private val CALL_TERMINAL_EVENTS = arrayOf("Missed Call", "Call Ended")
...
//    //endregion
    ...
    override fun onListen(arguments: Any?, events: EventSink?) {
        ...
        if (pendingEvents.isNotEmpty()) {
            ...
            buffered = collapseBufferedCallEvents(pendingEvents.toList())
            pendingEvents.clear()
            buffered.forEach { events.success(it) }
        }

        if (buffered.none { it.isCallStateEvent() } && buffered.none { it.isTerminalCallEvent() }) {
            emitActiveCallState()
        }
    }
    ...
    private fun String.isCallStateEvent(): Boolean =
        CALL_STATE_EVENT_PREFIXES.any { this == it || startsWith("$it|") }

    private fun String.isTerminalCallEvent(): Boolean =
        CALL_TERMINAL_EVENTS.any { this == it || startsWith("$it|") }

    private fun collapseBufferedCallEvents(events: List<String>): List<String> {
        if (events.none { it.isTerminalCallEvent() }) return events
        IncomingCallNotifier.cancel(context ?: return events.filterNot { it.isCallStateEvent() })
        return events.filterNot { it.isCallStateEvent() }
    }
...
//        return tm.isOnCall(ctx)
    ...
    private fun toggleSpeaker(ctx: Context, speakerIsOn: Boolean) {
        ...
        if (speakerIsOn) isBluetoothOn = false
        Intent(ctx, TVConnectionService::class.java).apply {
    ...
    private fun registerPhoneAccount(): Boolean {
        context?.let { ctx ->
            telecomManager?.let { tm ->
                ...
                // Always (re)register so a previously installed CALL_PROVIDER account is
                // replaced with CAPABILITY_SELF_MANAGED. registerPhoneAccount overwrites the
                // handle; TelecomManagerExtension also unregisters the old system-managed one.
                val phoneAccount = tm.getPhoneAccount(phoneAccountHandle)
                if (phoneAccount != null &&
                    phoneAccount.hasCapabilities(android.telecom.PhoneAccount.CAPABILITY_SELF_MANAGED) &&
    ...
    private fun logEvent(
        ...
        if (sink == null) {
            ...
            if (message.isTerminalCallEvent()) {
                pendingEvents.removeAll { it.isCallStateEvent() || it.isTerminalCallEvent() }
            } else if (message.isCallStateEvent()) {
    ...
    private fun checkIsPhoneAccountEnabled(): Boolean {
        context?.let { ctx ->
            telecomManager?.let { tm ->
                ...
                val account = tm.getPhoneAccount(phoneAccountHandle) ?: return false
                // Samsung often leaves self-managed VoIP out of the SIM list.
                return account.isEnabled ||
                    account.hasCapabilities(android.telecom.PhoneAccount.CAPABILITY_SELF_MANAGED)
            } ?: run {
                Log.e(TAG, "Telecom Manager is null, cannot check if registered phone account")
                return false
            }
