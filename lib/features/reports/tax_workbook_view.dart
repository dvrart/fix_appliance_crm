...
class TaxWorkbookFigures {
  ...
  /// Часть продаж периода, за которую заплатили наличными (без HST).
  /// Когда [includesCash] = false, эта сумма уже вычтена из цифр выше.
  final double cashSalesExHst;

  /// Входят ли наличные в цифры. False — выгрузка только по банку и Stripe.
  final bool includesCash;
  ...
  bool get hasCash => cashSalesExHst > 0.009;
...
class TaxWorkbookView extends StatelessWidget {
  ...
  /// Те же цифры без наличных — для второй кнопки выгрузки.
  final TaxWorkbookFigures figuresWithoutCash;
  ...
  const TaxWorkbookView({
    ...
    required this.figuresWithoutCash,
    required this.onSwitchToYear,
    this.onOpenExpenses,
    this.onScanReceipt,
  });

  /// Одна строка, по которой сразу видно, что это за файл: полный или
  /// только безналичный. Без неё две выгрузки за один год не различить.
  static List<String> _scopeLines(TaxWorkbookFigures f) {
    String m(double v) => v.toStringAsFixed(2);
    if (f.includesCash) {
      return [
        'SCOPE: ALL payment methods (cash, e-Transfer, Stripe).',
        if (f.hasCash) 'Of that, paid in cash excl. HST: ${m(f.cashSalesExHst)}',
      ];
    }
    return [
      'SCOPE: BANK AND STRIPE ONLY — CASH IS NOT INCLUDED.',
      'Cash sales excluded from every number below: ${m(f.cashSalesExHst)}',
      'This is a reconciliation view against the bank, not a full income '
          'statement. T2 and GST34 must report total income including cash.',
    ];
  }
  ...
  static Future<void> sharePdf(TaxWorkbookFigures f) async {
    ...
    final suffix = f.includesCash ? '' : '_bank_only';
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'Fix_Appliance_T2_workbook_$year$suffix.pdf',
    );
  }

  /// Спросить, что выгружать: всё или только безналичное.
  /// Возвращает false, если выбор отменили.
  static Future<bool> shareWithCashChoice(
    BuildContext context,
    TaxWorkbookFigures withCash,
    TaxWorkbookFigures withoutCash, {
    bool copyInstead = false,
  }) async {
    String money(double v) => '\$${v.toStringAsFixed(2)}';

    Future<bool> run(TaxWorkbookFigures f) async {
      if (copyInstead) {
        await Clipboard.setData(ClipboardData(text: clipboardText(f)));
        return true;
      }
      await sharePdf(f);
      return true;
    ...
    if (!withCash.hasCash) return run(withCash);
        ...
        child: Column(
          ...
          children: [
            Padding(
              ...
              child: Text(
                sheet.tr('Что выгрузить', 'What to export'),
            ...
            _exportOption(
              ...
              title: sheet.tr('С наличными', 'With cash'),
              subtitle: sheet.tr(
                'Всё, что заработано. Продажи ${money(withCash.salesExHst)}, HST ${money(withCash.hstCollectible)}.',
                'Everything earned. Sales ${money(withCash.salesExHst)}, HST ${money(withCash.hstCollectible)}.',
              ),
            ...
            _exportOption(
              ...
              title: sheet.tr('Без наличных', 'Without cash'),
              subtitle: sheet.tr(
                'Только банк и Stripe. Продажи ${money(withoutCash.salesExHst)}, HST ${money(withoutCash.hstCollectible)}. Наличных на ${money(withCash.cashSalesExHst)} здесь не будет.',
                'Bank and Stripe only. Sales ${money(withoutCash.salesExHst)}, HST ${money(withoutCash.hstCollectible)}. ${money(withCash.cashSalesExHst)} of cash is left out.',
              ),
            ...
            Padding(
              ...
              child: Text(
                sheet.tr(
                  'Вариант без наличных — сверка с банком. В T2 и GST34 доход показывают полностью, вместе с наличными.',
                  'The bank-only version is for reconciliation. T2 and GST34 must report all income, cash included.',
                ),
                style: const TextStyle(
    ...
    return run(choice ? withCash : withoutCash);
  }
          ...
          children: [
            ...
            Expanded(
              child: OutlinedButton.icon(
                onPressed: () async {
                  final done = await shareWithCashChoice(
                    context,
                    f,
                    figuresWithoutCash,
                    copyInstead: true,
            ...
            Expanded(
              child: ElevatedButton.icon(
                onPressed: () =>
                    shareWithCashChoice(context, f, figuresWithoutCash),
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFF14557F),
                  foregroundColor: Colors.white,
                ),
                icon: const Icon(Icons.ios_share),
                label: Text(context.tr('Скачать PDF', 'Download PDF')),
              ),
            ),
          ],
        ),
    ...
    return _card(
      child: Column(
        children: [
          ...
          if (f.hasCash)
            _moneyRow(
              context.tr('из них наличными', 'of that, paid in cash'),
              f.cashSalesExHst,
              muted: true,
            ),
        ...
        build: (context) => [
          ...
          pw.Container(
            ...
            decoration: pw.BoxDecoration(
              color: PdfColor.fromInt(f.includesCash ? 0xFFEFF5FA : 0xFFFFF4E0),
              border: pw.Border.all(
                color: PdfColor.fromInt(
                  f.includesCash ? 0xFF14557F : 0xFFCC8800,
                ),
                width: 0.8,
              ),
            ),
            child: pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                for (final line in _scopeLines(f))
                  pw.Text(
                    line,
                    style: pw.TextStyle(font: bold, fontSize: 9, lineSpacing: 2),
                  ),
              ],
            ),
          ),
