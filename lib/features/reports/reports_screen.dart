...
class _ReportsScreenState extends State<ReportsScreen> {
  ...
  TaxWorkbookFigures _figures(_ReportMetrics metrics) {
    return TaxWorkbookFigures(
      ...
      salesExHst: metrics.salesExHst,
      hstCollectible: metrics.hstCollectible,
      cashExHst: metrics.cashExHst,
      hstReceived: metrics.hstReceived,
      partsCost: metrics.expenses,
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          if (_t2Mode) ...[
            IconButton(
              ...
              onPressed: _latestMetrics == null
                  ...
                  : () {
                      Clipboard.setData(
                        ClipboardData(
                          text: TaxWorkbookView.clipboardText(
            ...
            IconButton(
              tooltip: context.tr('PDF для подачи', 'PDF for filing'),
              onPressed: _latestMetrics == null
                  ? null
                  : () => TaxWorkbookView.sharePdf(_figures(_latestMetrics!)),
  ...
  _ReportMetrics _calculateMetrics(List<DocumentSnapshot> docs) {
    ...
    void addCash(double amount, double subtotal, double tax, DateTime when) {
      ...
      for (final inv in job.documents) {
        ...
        if (payments is List) {
          for (final payment in payments) {
            ...
            jobsInPeriod.add(job.id);
            addCash(amount, subtotal, tax, payDate!);
          }
        }
        if (attributedPaid == 0 &&
            paidOnDoc > 0 &&
