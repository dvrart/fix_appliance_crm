...
/// Ловит wake-слово (Purysh / purish / пурыш).
/// Сначала Android STT (с выключенным системным писком), иначе непрерывный PCM.
class WakeWordService extends ChangeNotifier {
  ...
  static const _speechRms = 0.0045;
  static const _silenceRms = 0.0028;
  ...
  bool _tokenMatches(String word, Set<String> phrases) {
    ...
    for (final phrase in phrases) {
      ...
      final maxDist = phrase.length <= 5 ? 1 : 2;
  ...
  Future<void> start() async {
    ...
    _enabled = true;
    final sttOk = await _listenStt();
    if (!sttOk && _enabled && !_waking) {
      await _listenPcm();
    }
  }
  ...
  Future<void> _muteBeeps(bool mute) async {
    try {
      await _deviceChannel.invokeMethod('muteRecognitionBeeps', mute);
    } catch (_) {}
  }
  ...
  Future<bool> _listenStt() async {
    if (!_enabled || _waking) return false;
    _starting = true;
    final epoch = _epoch;
    try {
      final mic = await Permission.microphone.request();
      if (!mic.isGranted || !_enabled || epoch != _epoch) return false;
      final ok = await _speech.initialize(
        onError: (error) {
          debugPrint('WakeWord STT error ${error.errorMsg}');
          if (_useStt) _scheduleSttRestart();
        },
        onStatus: (status) {
          debugPrint('WakeWord STT $status');
          if (!_useStt || !_enabled || _waking) return;
      ...
      if (!ok || !_enabled || epoch != _epoch) return false;
      _useStt = true;
      await _beginSttListen();
      await Future<void>.delayed(const Duration(milliseconds: 400));
      if (!_enabled || epoch != _epoch) return false;
      if (!_speech.isListening) {
        debugPrint('WakeWord STT не слушает — PCM');
        _useStt = false;
        try {
          await _speech.stop();
        } catch (_) {}
  ...
  Future<void> _beginSttListen() async {
    if (!_enabled || _waking) return;
    if (_speech.isListening) return;
    try {
      await _muteBeeps(true);
      await _speech.listen(
        onResult: (result) {
          if (!_enabled || _waking) return;
          final text = result.recognizedWords;
          if (text.trim().isEmpty) return;
          debugPrint('WakeWord STT: $text');
          if (matchesWake(text)) unawaited(_fireWake());
        },
        listenOptions: SpeechListenOptions(
          partialResults: true,
          cancelOnError: false,
          listenMode: ListenMode.dictation,
          listenFor: const Duration(minutes: 55),
      ...
      Future<void>.delayed(const Duration(milliseconds: 700), () {
        if (!_waking) unawaited(_muteBeeps(false));
      });
    }
  }

  void _scheduleSttRestart() {
    if (!_enabled || _waking || !_useStt) return;
    _reconnectTimer?.cancel();
    _reconnectTimer = Timer(const Duration(milliseconds: 900), () {
      if (_enabled && !_waking && _useStt && !_speech.isListening) {
        unawaited(_beginSttListen());
      }
    });
  }
  ...
  Future<void> _listenPcm() async {
    ...
    try {
      final mic = await Permission.microphone.request();
      if (!mic.isGranted || !_enabled || epoch != _epoch) return;
      ...
      _setArmed(true);
      debugPrint('WakeWord: PCM слушаю «$_wakeWord»');
    } catch (e) {
  ...
  void _schedulePcmReconnect() {
    if (!_enabled || _waking || _starting || _useStt) return;
    _reconnectTimer?.cancel();
    ...
    for (final name in _probeModels) {
      try {
        ...
        final response = await model.generateContent([
          Content.multi([
            TextPart(
              'The wake name is Purysh (sounds like purish, purush, purge, '
              'parish, puresh, пурыш, пуриш). '
  ...
  ({String text, bool wake}) _parseWakeReply(String raw) {
    ...
    if (text.isEmpty) {
      text = raw
          .replaceAll(RegExp(r'w:\s*(yes|no|да|нет)', caseSensitive: false), '')
