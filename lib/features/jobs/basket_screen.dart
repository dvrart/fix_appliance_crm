...
class _BasketScreenState extends State<BasketScreen>
  ...
  void initState() {
    ...
    WarehouseService.purgeExpiredTrash();
  }
  ...
  Future<void> _restoreWarehouseItem(WarehouseItem item) async {
    await WarehouseService.restore(item.id);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          context.tr('Позиция восстановлена на склад', 'Item restored to warehouse'),
        ),
      ),
    );
  }

  Future<void> _forgetWarehouseItem(WarehouseItem item) async {
    final ok = await _confirmForever(
      context.tr(
        'Удалить позицию со склада навсегда?',
        'Delete this warehouse item forever?',
      ),
    );
    if (ok != true) return;
    AppCommands.reactAngry();
    await WarehouseService.deleteForever(item.id);
  }
  ...
  Future<void> _forgetAllWarehouseItems(List<WarehouseItem> items) async {
    final ok = await _confirmForever(
      context.tr(
        'Удалить все позиции склада навсегда?',
        'Delete all warehouse items forever?',
      ),
    );
    if (ok != true || _busy) return;
    AppCommands.reactAngry();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            Tab(text: context.tr('Склад', 'Warehouse')),
      ...
      body: TabBarView(
        ...
        children: [
          ...
          StreamBuilder<List<WarehouseItem>>(
            stream: WarehouseService.streamTrashed(),
            builder: (context, snap) {
              final items = snap.data ?? const <WarehouseItem>[];
              if (!snap.hasData) {
                return const Center(child: CircularProgressIndicator());
              }
              if (items.isEmpty) {
                return _empty(
                  context.tr(
                    'Нет удалённых позиций склада. Удалите запчасть на складе — она будет здесь 30 дней.',
                    'No deleted warehouse items. Delete a part on the warehouse screen and it stays here for 30 days.',
                  ),
                );
              }
              return ListView(
                ...
                children: [
                  _hint(),
                  _deleteAllButton(() => _forgetAllWarehouseItems(items)),
                  for (final item in items)
                    Card(
                      margin: const EdgeInsets.only(bottom: 8),
                      child: ListTile(
                        ...
                        subtitle: Text(
                          [
                            ...
                            '${context.tr('Кол-во', 'Qty')}: ${item.quantity}',
                            Formatters.formatCurrency(item.price),
                            _daysLeft(item.trashDaysLeft),
                          ].where((part) => part.trim().isNotEmpty).join(' · '),
                        ),
                        trailing: _actions(
                          onRestore: () => _restoreWarehouseItem(item),
                          onForget: () => _forgetWarehouseItem(item),
                        ),
                      ),
                    ),
                ],
