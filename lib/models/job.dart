...
class JobVisit {
  ...
  JobVisit copyWith({
    ...
    return JobVisit(
      ...
      smsDialog: clearSms ? '' : (smsDialog ?? this.smsDialog),
    );
  }
...
class Job {
  ...
  String displayStatusForVisit(JobVisit? visit) {
    ...
    if (latest == null || visit.id == latest.id) return status;
  ...
  /// Invoice payment mark for lists: `paid`, `deposit`, `unpaid`. Empty for estimates.
  static String documentPayMark(Map doc) {
    ...
    if (mode == 'deposit' || (paid > 0.009 && due > 0.009)) return 'deposit';
    return 'unpaid';
  }
