...
class StatusColorGroup {
  const StatusColorGroup({required this.labelKey, required this.colors});
...
class StatusService {
  ...
  static const Map<String, int> _builtinColors = {
    ...
    JobStatuses.repeatVisit: 0xFF5C6BC0,
  ...
  /// Палитра для редактора статусов — оттенки по группам.
  static const List<StatusColorGroup> statusColorGroups = [
    StatusColorGroup(
      labelKey: 'Синий',
      colors: [
        0xFF0D47A1,
  ...
  static List<int> get statusColorPalette => [
    for (final id in builtins) _builtinColors[id] ?? 0xFF14557F,
    for (final group in statusColorGroups) ...group.colors,
  ];
  ...
  static Future<void> replaceAll(List<JobStatusDef> items) => _save(items);
  ...
  static Future<void> add(String status, {int? colorValue}) async {
    ...
    var color = colorValue ?? statusColorPalette.first;
    for (final candidate in statusColorPalette) {
