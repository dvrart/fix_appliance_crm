...
class JobService {
  ...
  static Stream<List<Job>> streamNeedsReview() {
    return _ref.where('needsReview', isEqualTo: true).snapshots().map((
      ...
      return _collapseDuplicateEmailJobs(jobs);
    });
  }

  static String _emailJobDedupeKey(Job job) {
    if (job.intakeSource != 'email') return '';
    ...
    final name = job.clientName.trim().toLowerCase();
    return '${from.isEmpty ? name : from}|$minute|$desc';
  }

  static List<Job> _collapseDuplicateEmailJobs(List<Job> jobs) {
    if (jobs.length < 2) return jobs;
    final seen = <String>{};
    final out = <Job>[];
    for (final job in jobs) {
      final key = _emailJobDedupeKey(job);
      if (key.isEmpty || key == '||') {
        out.add(job);
        continue;
      }
  ...
  /// ответа сервера, а нам нужно сразу открыть карточку.
  ...
  static Future<void> updateStatus(
    ...
    await settleWrite(_ref.doc(id).update(updates));
  }
  ...
  static Future<void> patchCallNotes({
    ...
    try {
      ...
      final next = raw.map((item) {
        ...
        changed = true;
        final nextT = transcription.trim();
        final prevT = (map['transcription'] ?? '').toString();
        map['transcription'] = nextT.length >= prevT.length ? nextT : prevT;
        if (summary.trim().isNotEmpty) map['summary'] = summary;
        if (transcriptionRu != null) {
          final nextRu = transcriptionRu.trim();
          final prevRu = (map['transcriptionRu'] ?? '').toString();
          map['transcriptionRu'] =
              nextRu.length >= prevRu.length ? nextRu : prevRu;
        }
        if (transcriptionEn != null) {
          final nextEn = transcriptionEn.trim();
          final prevEn = (map['transcriptionEn'] ?? '').toString();
          map['transcriptionEn'] =
              nextEn.length >= prevEn.length ? nextEn : prevEn;
        }
        return map;
      }).toList();
      if (!changed) return;
  ...
  static Future<void> saveVisits(
    ...
    if (markInstall && status == JobStatuses.waitingPart) {
      data['status'] = JobStatuses.install;
