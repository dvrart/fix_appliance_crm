...
class ImportExportService {
  ...
  /// Полный слепок клиентов и заявок — и для ручного экспорта, и для автокопии.
  ...
  static Future<void> exportBackupJson() async {
    await _saveOrShare('fix_backup.json', await buildBackupJson());
  }
  ...
  static Future<ImportResult> importFromFile({String? sourceLabel}) async {
    ...
    try {
      ...
      return await _importCsv(text, sourceLabel: sourceLabel);
    } catch (e) {
  ...
  static Future<ImportResult> _importCsv(
    ...
    for (var i = 1; i < table.length; i++) {
      ...
      final result = await _upsertClient({
        ...
        'companyName': company,
        'notes': notes,
        'source': source.isNotEmpty ? source : (sourceLabel ?? ''),
      });
      if (result == _Upsert.created) clientsCreated += 1;
      if (result == _Upsert.updated) clientsUpdated += 1;
  ...
  static Future<void> _createJob(Map<String, dynamic> map) async {
    ...
    final duration = int.tryParse('${map['durationMinutes'] ?? ''}') ?? kDefaultVisitMinutes;
