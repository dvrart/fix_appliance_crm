/// English UI strings. Keys are the original Russian texts shown in the app.
const Map<String, String> kEnglishUi = {
  'Заявки': 'Jobs',
  'Календарь': 'Calendar',
  'Клиенты': 'Clients',
  'Клиент': 'Client',
  'Настройки': 'Settings',
  'Компания': 'Company',
  'Связь': 'Communication',
  'Счета': 'Invoices',
  'Сметы': 'Estimates',
  'Скопировано': 'Copied',
  'Входящий': 'Incoming',
  'Исходящий': 'Outgoing',
  'Нумерация': 'Numbering',
  'Префикс номера': 'Number prefix',
  'Следующий счёт': 'Next invoice',
  'Следующая смета': 'Next estimate',
  'Нет счетов': 'No invoices',
  'Нет смет': 'No estimates',
  'Удалить счёт?': 'Delete this invoice?',
  'Удалить смету?': 'Delete this estimate?',
  'Счёт попадёт в корзину.': 'The invoice will go to the trash.',
  'Смета попадёт в корзину.': 'The estimate will go to the trash.',
  'Оставить': 'Keep',
  'Изменить имя': 'Edit name',
  'Изменить компанию': 'Edit company',
  'Финансы': 'Finance',
  'Финансы и документы': 'Finance & documents',
  'Ассистент': 'Assistant',
  'Зона обслуживания': 'Service area',
  'Каталог': 'Catalog',
  'Меню приложения': 'App menu',
  'О приложении': 'About',
  'Язык': 'Language',
  'Версия': 'Version',
  'Версия 1.0.0': 'Version 1.0.0',
  'Актуальная': 'Current',
  'Учёт заявок, звонков и счетов': 'Jobs, calls, and invoices',
  'Звонки': 'Calls',
  'Сообщения': 'Messages',
  'Переписка': 'Inbox',
  'Написать': 'Message',
  'Написать письмо...': 'Write an email...',
  'Написать SMS...': 'Write an SMS...',
  'SMS и письма от клиентов\nпоявятся здесь': 'SMS and emails from clients\nwill appear here',
  'SMS от клиентов\nпоявятся здесь': 'SMS from clients\nwill appear here',
  'Письма от клиентов\nпоявятся здесь': 'Emails from clients\nwill appear here',
  'Нет SMS — напишите первым': 'No SMS yet — write first',
  'Нет писем — напишите первым': 'No emails yet — write first',
  'Нет email у клиента': 'This client has no email',
  'Нет телефона у клиента': 'This client has no phone number',
  'Нет телефона или email.\nДобавьте контакт клиента.':
      'No phone or email.\nAdd a client contact.',
  'От': 'From',
  'Вы': 'You',
  'Новое письмо': 'New email',
  'Укажите Gmail': 'Enter a Gmail address',
  'Gmail сохранён': 'Gmail saved',
  'Сохранить Gmail': 'Save Gmail',
  'Отслеживание писем': 'Watch emails',
  'Письма с этих адресов попадут в CRM, даже если это не ответ клиенту.':
      'Mail from these addresses is saved in the CRM even if it is not a client reply.',
  'Заявки составляются только с этих адресов.':
      'Jobs are built only from these addresses.',
  'Заявки составляются только с этих адресов. Имя переписки — как нить показывается в чате.':
      'Jobs are built only from these addresses. Conversation name is how the thread appears in chat.',
  'Добавить адрес отправителя': 'Add sender address',
  'Адрес отправителя': 'Sender address',
  'Имя переписки': 'Conversation name',
  'Например Jobber': 'e.g. Jobber',
  'Укажите email отправителя': 'Enter a sender email',
  'Этот адрес уже в списке': 'This address is already on the list',
  'Unit': 'Unit',
  'Пароль приложения': 'App password',
  'Новый пароль приложения': 'New app password',
  'Оставьте пустым, чтобы не менять': 'Leave empty to keep the current password',
  'В CRM попадают ответы клиентов на ваши письма. Письма про ремонт бытовой техники тоже становятся заявкой. Если в теме есть заданная фраза — заявка создаётся сразу. Пароль — «Пароль приложения» Google, не обычный пароль от почты.':
      'Client replies appear in chats. Emails about appliance repair also become jobs. If the subject contains the phrase you set, a job is created right away. Use a Google App Password, not your regular Gmail password.',
  'Каждое новое письмо проверяется. Если там ремонт бытовой техники — придёт уведомление, и можно создать карточку клиента и встречу. Ответы клиентов на ваши письма попадают в переписку. Пароль — «Пароль приложения» Google, не обычный пароль от почты.':
      'Every new email is checked. If it is about appliance repair, you get a notification and can create a client card and a visit. Client replies to your letters appear in chat. Use a Google App Password, not your regular Gmail password.',
  'Заголовок для заявок с почты': 'Email subject for new jobs',
  'Если тема содержит эту фразу — заявка. Письма про ремонт техники тоже разбираются.':
      'If the subject contains this phrase — a job is created. Appliance-repair emails are scanned too.',
  'Если тема содержит эту фразу — письмо сразу идёт как заявка на проверку. Письма про ремонт техники тоже разбираются.':
      'If the subject contains this phrase, the letter is offered as a job. Appliance-repair emails are scanned too.',
  'Карточка клиента': 'Client card',
  'Нет карточки клиента': 'No client card',
  'Нажмите, чтобы вернуться. Можно положить трубку.':
      'Tap to return. You can hang up from here.',
  'История звонков': 'Call history',
  'Звонков пока нет': 'No calls yet',
  'Аудио ещё готовится': 'Audio is still preparing',
  'Без подписи': 'Skip signature',
  'Проверяются только письма с адресов из списка ниже. Из них составляется заявка. Ответы клиентов на ваши письма попадают в переписку. Пароль — «Пароль приложения» Google, не обычный пароль от почты.':
      'Only letters from the addresses below are checked. Jobs are created from them. Client replies to your letters appear in chat. Use a Google App Password, not your regular Gmail password.',
  'Письмо о ремонте': 'Repair email',
  'Укажите телефон или email': 'Enter a phone or email',
  'Нет текста письма': 'No email text',
  'Письмо · создать': 'Email · create',
  'Заявки по времени': 'Jobs over time',
  'Нет заявок за этот период': 'No jobs in this period',
  'Инвойсы': 'Invoices',
  'Оплаты': 'Payments',
  'Визиты': 'Visits',
  'Письма': 'Emails',
  'Не удалось перевести на английский. Сообщение не отправлено.':
      'Could not translate to English. The message was not sent.',
  'Запись ещё не готова': 'Recording is not ready yet',
  'Какие плитки показывать в боковом меню. Звонки и переписка — в нижней вкладке «Связь». Настройки всегда остаются.':
      'Which tiles to show in the side menu. Calls and messages are in the Communication tab. Settings always stay.',
  'Склад': 'Warehouse',
  'Запчасти': 'Parts',
  'Отчеты': 'Reports',
  'Статистика': 'Statistics',
  'Нажмите логотип — настройки': 'Tap the logo for settings',
  'Сегодня': 'Today',
  'Завтра': 'Tomorrow',
  'Вчера': 'Yesterday',
  'В работе': 'In progress',
  'Поиск везде': 'Search everywhere',
  'Все': 'All',
  'Показать все': 'Show all',
  'Неоплачено': 'Unpaid',
  'Неоплаченные': 'Unpaid',
  'Сделано': 'Done',
  'Готов': 'Ready',
  'Фильтры списка': 'List filters',
  '1-й фильтр': '1st filter',
  '2-й фильтр': '2nd filter',
  '3-й фильтр': '3rd filter',
  'Вызов': 'Call-out',
  'Ожидание запчасти': 'Waiting for part',
  'Перенос': 'Rescheduled',
  'Перенесено': 'Moved',
  'Завершено': 'Completed',
  'Отменено': 'Cancelled',
  'Отмена': 'Cancel',
  'Сохранить': 'Save',
  'Не сохранять': 'Don\'t save',
  'Сохранить изменения?': 'Save changes?',
  'Без подтверждения изменения не сохранятся.':
      'Changes will not be saved unless you confirm.',
  'Где работа': 'Job site',
  'Другой адрес работы': 'Different job-site address',
  'Адрес работы (куда ехать)...': 'Job-site address (where to go)...',
  'Укажите имя, телефон и адрес работы': 'Enter name, phone and job-site address',
  'Убрать': 'Remove',
  'Сохранение…': 'Saving…',
  'Сохранение...': 'Saving...',
  'Не удалось сохранить': 'Could not save',
  'Готово': 'Done',
  'Открыть': 'Open',
  'Удалить': 'Delete',
  'Удалить?': 'Delete?',
  'Добавить': 'Add',
  'Добавить фотографии': 'Add photos',
  'Список пуст — добавьте позицию': 'The list is empty — add an item',
  'Повторить': 'Retry',
  'Проверить': 'Review',
  'Проверено': 'Reviewed',
  'Далее': 'Next',
  'Назад': 'Back',
  'Ошибка': 'Error',
  'Загрузка...': 'Loading...',
  'Не указан': 'Not set',
  'Не указано': 'Not set',
  'Не указана': 'Not set',
  'Нет': 'No',
  'Список пуст': 'The list is empty',
  'Ничего не найдено': 'Nothing found',
  'Нельзя удалить клиента': 'Cannot delete this client',
  'Пока есть заявки': 'This client still has jobs',
  'Карточку удалить нельзя. Сначала удалите работы или перенесите их другому клиенту.':
      'The card cannot be deleted. Delete or move the jobs first.',
  'Понятно': 'OK',
  'Удалить контакт': 'Delete contact',
  'Контакт удалён': 'Contact deleted',
  'не удалены — есть заявки': 'not deleted — they still have jobs',
  'Контакт': 'Contact',
  'Поиск или новое значение...': 'Search or add a new value...',
  'Новое значение': 'New value',
  'Тип техники': 'Appliance type',
  'Типы техники': 'Appliance types',
  'Типы техники и бренды': 'Appliance types and brands',
  'Бренд': 'Brand',
  'Бренды': 'Brands',
  'Список для создания заявки': 'Used when creating a job',
  'Эти значения предлагаются в поле «Тип»': 'These values appear in the Type field',
  'Эти значения предлагаются в поле «Бренд»': 'These values appear in the Brand field',
  'Вид по умолчанию': 'Default view',
  'Календарь и время': 'Calendar & time',
  'Выходные дни': 'Weekends',
  'Показывать субботу и воскресенье': 'Show Saturday and Sunday',
  'Начало недели': 'First day of week',
  'Понедельник': 'Monday',
  'Воскресенье': 'Sunday',
  'Источник времени': 'Time source',
  'Рабочее время': 'Working hours',
  'Длительность визита': 'Visit duration',
  'Начало рабочего дня': 'Start of workday',
  'Конец рабочего дня': 'End of workday',
  'Конец рабочего дня должен быть позже начала': 'End of the workday must be after the start',
  'Вручную': 'Manual',
  'Вручную · время телефона': 'Manual · phone time',
  'Время телефона': 'Phone time',
  'По геолокации': 'By location',
  'Часовой пояс по GPS': 'Time zone from GPS',
  'Подождите, определяется пояс по GPS': 'Wait, detecting time zone from GPS',
  'Сначала определите время по геолокации': 'First detect the time from location',
  'Не удалось определить время по GPS. Проверьте геолокацию.':
      'Could not detect time from GPS. Check location access.',
  'Не удалось определить время по GPS': 'Could not detect time from GPS',
  'нет': 'none',
  '15 мин': '15 min',
  '20 мин': '20 min',
  '30 мин': '30 min',
  '30 минут': '30 minutes',
  '45 мин': '45 min',
  '45 минут': '45 minutes',
  '1 час': '1 hour',
  '1.5 ч': '1.5 h',
  '1.5 часа': '1.5 hours',
  '2 часа': '2 hours',
  '3 часа': '3 hours',
  'Название компании': 'Company name',
  'Название в меню': 'Name in the menu',
  'Телефон': 'Phone',
  'Сайт': 'Website',
  'Отменить заявку?': 'Cancel this job?',
  'Заявка попадёт в корзину на 30 дней.': 'The job goes to the trash for 30 days.',
  'Телефон компании': 'Company phone',
  'Телефон в счетах и SMS': 'Phone on invoices and SMS',
  'Email компании': 'Company email',
  'Почта в счетах': 'Email on invoices',
  'Адрес': 'Address',
  'Адрес компании': 'Company address',
  'Адрес в счетах и PDF': 'Address on invoices and PDFs',
  'Адрес клиента': 'Client address',
  'Нажмите на логотип, чтобы заменить': 'Tap the logo to replace it',
  'Не удалось сохранить логотип': 'Could not save the logo',
  'Название, логотип, телефон, адрес': 'Name, logo, phone, address',
  'Реквизиты компании': 'Company details',
  'Звонки Twilio': 'Twilio calls',
  'Звонки не работают': 'Calls are not working',
  'Разрешите приложению свои звонки, чтобы входящие открывались в CRM':
      'Allow this app to handle phone calls so incoming calls open in the CRM',
  'Входящие принимаются в приложении': 'Incoming calls are handled in the app',
  'ИИ берёт трубку': 'AI answers the phone',
  'Если не ответить за 20 секунд, ИИ сам примет заказ':
      'If you do not pick up within 20 seconds, AI takes the request',
  'Чему учиться': 'What to learn',
  'Самообучение по вашему «можно»': 'Learn only what you approve',
  'После звонка пришлёт, чему хочет научиться. В скрипт попадёт только после вашего подтверждения.':
      'After a call it sends what it wants to learn. Nothing enters the script until you confirm.',
  'Подтвердить или сказать «не надо»': 'Approve or say no',
  'Обучение': 'Learning',
  'Ждёт вашего «можно»': 'Waiting for your yes',
  'Разобрано': 'Reviewed',
  'Можно, выучи': 'Yes, learn it',
  'Только это': 'Only this',
  'Не надо': 'No need',
  'Выучено': 'Learned',
  'Из разговора': 'From the call',
  'Секретарь хочет научиться': 'The secretary wants to learn',
  'Разбор секретаря': 'Secretary review',
  'Разобрать секретаря': 'Review the secretary',
  'Разобрать ошибку секретаря': 'Review the secretary error',
  'Нет текста разговора': 'There is no call text',
  'Так и надо': 'That was fine',
  'Сохранить правку': 'Save the correction',
  'Написать правку самому': 'Write a correction yourself',
  'Все предложения': 'All proposals',
  'Можно уточнить: выучи это, остальное не надо…':
      'Optional: learn this, nothing else…',
  'После звонка секретарь пришлёт, чему хочет научиться. Скрипт не меняется, пока вы не скажете «можно».':
      'After a call the secretary will send what it wants to learn. The script does not change until you say yes.',
  'Критерии из настроек всегда главнее. Вы подтверждаете только привычки из реальных звонков.':
      'Your criteria always win. You only confirm habits from real calls.',
  'Обучение секретаря': 'Secretary learning',
  'Разбор звонка секретаря': 'Secretary call review',
  'Полный отчёт ошибки секретаря — вы пишете, как действовать дальше':
      'Full secretary error report — you write how it should act next time',
  'Секретарь хочет научиться — сначала ваше «можно»':
      'The secretary wants to learn — your yes comes first',
  'Критерии секретаря': 'Secretary criteria',
  'Что спрашивать, зона, приветствие и тон':
      'What to ask, service area, greeting and tone',
  'Секретарь на звонках': 'Phone secretary',
  'Правила секретаря сохранены': 'Secretary rules saved',
  'Сохранить правила секретаря': 'Save secretary rules',
  'ИИ ответил': 'AI answered',
  'ИИ взял звонок': 'AI answered the call',
  'ИИ создал заявку': 'AI created a job',
  'ИИ-ДИСПЕТЧЕР': 'AI DISPATCHER',
  'ИИ говорит с клиентом только если вы не взяли трубку. Голос живой, как у человека. Здесь задайте, что спрашивать и как себя вести.':
      'The AI talks to the caller only if you miss the call. The voice sounds like a real person. Write what to ask and how to behave.',
  'Приветствие': 'Greeting',
  '{company} — название компании': '{company} is the company name',
  'Правила разговора': 'Conversation rules',
  'Что спрашивать, чего не обещать, какой тон':
      'What to ask, what not to promise, what tone to use',
  'Сохранить правила ИИ': 'Save AI rules',
  'Правила ИИ сохранены': 'AI rules saved',
  'Проверка статуса…': 'Checking status…',
  'Разрешения звонков Android': 'Android call permissions',
  'Системный экран учётки телефона приложения': 'System screen for the app phone account',
  'Шапка SMS': 'SMS header',
  'Без шапки': 'No header',
  'Первая строка каждого SMS': 'First line of every SMS',
  'После неё будет пустая строка. Например: fix-appliance.ca. Оставьте пустым, если шапка не нужна.':
      'A blank line follows it. Example: fix-appliance.ca. Leave empty if you do not need a header.',
  'Только эта строка ставится в начало каждого SMS. Название компании и FIX ApplianceCA туда не добавляются. Пример: fix-appliance.ca. Оставьте пустым, если шапка не нужна.':
      'Only this line is added at the start of every SMS. The company name and FIX ApplianceCA are not added. Example: fix-appliance.ca. Leave empty if you do not need a header.',
  'Подтверждена': 'Confirmed',
  'Не подтверждена': 'Not confirmed',
  'Отменена': 'Cancelled',
  'Язык ассистента': 'Assistant language',
  'Через 1.5 часа заявка': 'Job in 1.5 hours',
  'Скоро заявка': 'Upcoming job',
  'Напоминание за 1.5 часа до визита': 'Reminder 1.5 hours before the visit',
  'Клиент отменил заявку': 'The client cancelled the job',
  'Нужен перенос': 'Needs reschedule',
  'Заявка подтверждена': 'Job confirmed',
  'Заявка не подтверждена': 'Job not confirmed',
  'Подтверждение визита': 'Visit confirmation',
  'Можно поставить вручную, если клиент ответил по телефону.':
      'You can set this by hand if the client answered by phone.',
  'Клиент подтвердил или не подтвердил визит':
      'Client confirmed or did not confirm a visit',
  'Заявки в 7:00': 'Jobs at 7:00',
  'Заявки утром и вечером': 'Jobs morning and evening',
  'В 7:00 — сегодняшний день. В 19:00 — завтра и что взять.':
      'At 7:00 — today. At 19:00 — tomorrow and what to take.',
  'Выбор оплаты': 'Choose payment',
  'Начните вводить — поиск по складу': 'Start typing — search warehouse',
  'Ссылка Stripe': 'Stripe link',
  'Уведомление, что взять с собой на день': 'Notification of what to bring for the day',
  'SMS «я в пути»': 'On-the-way SMS',
  'Спросить после 2 км от клиента': 'Ask after 2 km from the client',
  'SMS-ШАБЛОНЫ': 'SMS TEMPLATES',
  'Шаблон «Я в пути»': '“On my way” template',
  'Шаблон «Ожидание запчасти»': '“Waiting for part” template',
  'Шаблон «Ремонт завершен»': '“Job done” template',
  'Сохранить шаблоны': 'Save templates',
  'Шаблоны сохранены': 'Templates saved',
  'Счета и сметы': 'Invoices & estimates',
  'Шаблоны SMS. PDF всегда на английском': 'SMS templates. PDFs are always in English',
  'Электронная подпись': 'E-signature',
  'Спрашивать подпись в конце работы': 'Ask for a signature at the end of the job',
  'Спрашивать подпись при создании счёта':
      'Ask for a signature when creating an invoice',
  'Расписка на счёте': 'Signature for the invoice',
  'Расписка на PDF счёта': 'Printed on the invoice PDF',
  'Подпись сохранена': 'Signature saved',
  'Клиент расписывается на этом телефоне. Для оплаты по SMS не нужна — можно Без подписи.':
      'The client signs on this phone. Skip it for a payment SMS.',
  'Не удалось сохранить подпись': 'Could not save the signature',
  'После запчасти новый визит ставит статус «Установка». Старые выезды на календаре остаются «Перенос».':
      'After a part, a new visit sets status to “Install”. Older visits stay “Rescheduled” on the calendar.',
  'После сохранения статус станет «Установка». Старый визит на календаре — «Перенос».':
      'After saving, the status will become “Install”. The old visit stays “Rescheduled” on the calendar.',
  '«Перенос» ставится сам, когда после запчасти вы назначаете новый визит.':
      '“Rescheduled” is set automatically when you book a new visit after a part.',
  'После сохранения статус станет «Перенос».':
      'After saving, the status will become “Rescheduled”.',
  'Сам, после нового визита': 'Automatic, after a new visit',
  'Больше не используется': 'No longer used',
  'Налог по умолчанию': 'Default tax',
  'HST 13%': 'HST 13%',
  'GST 5%': 'GST 5%',
  'Без налога': 'No tax',
  'Без налога (0%)': 'No tax (0%)',
  'Например, для First Nations': 'For example, First Nations',
  'В каждом счёте налог всё равно можно сменить': 'You can still change tax on each invoice',
  'HST / GST номер': 'HST / GST number',
  'Налоговый номер в счетах': 'Tax number on invoices',
  'Не указан — для счетов и PDF': 'Not set — used on invoices and PDFs',
  'Вызов только кнопкой микрофона': 'Open only with the microphone button',
  'Выключен': 'Off',
  'Как вызывать': 'How to open',
  'Большой микрофон в шапке. Кружок — пауза, тап в сторону — закрыть.':
      'Big microphone in the app bar. Tap the circle to pause, tap outside to close.',
  'Кнопка микрофона': 'Microphone button',
  'Ассистент выключен в настройках': 'Assistant is turned off in Settings',
  'Какие плитки показывать в боковом меню. Настройки всегда остаются.':
      'Which tiles to show in the side menu. Settings always stay.',
  'Какие разделы показывать в боковом меню': 'Which sections to show in the side menu',
  'Показано в меню': 'Shown in menu',
  'Скрыто': 'Hidden',
  'Провинция': 'Province',
  'Район ещё не отмечен': 'No area marked yet',
  'Отменить точку': 'Undo point',
  'Очистить': 'Clear',
  'Зона обслуживания сохранена': 'Service area saved',
  'Выберите провинцию и отметьте район на карте. Нажимайте по карте, чтобы поставить точки.':
      'Pick a province and mark your area on the map. Tap the map to drop points.',
  'Выберите провинцию и отметьте район на карте. Нажимайте по карте, чтобы поставить точки. Секретарь на звонках берёт зону отсюда.':
      'Pick a province and mark your area on the map. Tap the map to drop points. The phone secretary uses this area.',
  'Не отмечена на карте': 'Not marked on the map',
  'Район отмечен на карте': 'Area marked on the map',
  'Секретарь берёт зону с карты': 'Secretary uses the map area',
  'Не отмечена на карте. Откройте зону обслуживания и нарисуйте район.':
      'Not marked on the map. Open Service area and draw the region.',
  'Что спрашивать, приветствие и тон. Зона — с карты обслуживания.':
      'What to ask, greeting and tone. Coverage comes from the service-area map.',
  'Новая заявка': 'New job',
  'Новая заявка из звонка': 'New job from call',
  'Создать заявку': 'Create job',
  'ДАННЫЕ ВЛАДЕЛЬЦА (BILL TO)': 'OWNER (BILL TO)',
  'Номер телефона': 'Phone number',
  'Имя': 'Name',
  'Имя на английском': 'Name in English',
  'Имя владельца': 'Owner name',
  'Имя / Контактное лицо': 'Name / contact',
  'ТЕХНИКА И ПРОБЛЕМА': 'APPLIANCE & PROBLEM',
  'Тип (Холодильник и т.д.)': 'Type (fridge, washer…)',
  'Описание проблемы': 'Problem description',
  'Опишите проблему...': 'Describe the problem...',
  'ДАТА И ВРЕМЯ': 'DATE & TIME',
  'Отправить повторное уведомление смс': 'Resend visit notification SMS',
  'Нет телефона для SMS': 'No phone number for SMS',
  'Сначала нажмите «Проверено» — потом можно слать SMS.':
      'Tap “Reviewed” first — then you can send SMS.',
  'SMS с подтверждением визита отправлено': 'Visit confirmation SMS sent',
  'Дата визита': 'Visit date',
  'Выберите дату': 'Choose a date',
  'Выберите время': 'Choose a time',
  'Дата и время визита': 'Visit date and time',
  'Техника находится по другому адресу': 'The appliance is at another address',
  'КОНТАКТ НА МЕСТЕ (JOB SITE)': 'JOB SITE CONTACT',
  'Другое место работы (Арендатор)': 'Different job site (tenant)',
  'Другой адрес работы (арендатор)': 'Different job site (tenant)',
  'Имя на месте': 'Name on site',
  'Имя на месте (Арендатор)': 'Name on site (tenant)',
  'Телефон на месте': 'Phone on site',
  'Адрес работы (Куда ехать)...': 'Job address (where to go)...',
  'АДРЕС': 'ADDRESS',
  'Поиск адреса': 'Address search',
  'Начните вводить адрес (поиск)': 'Start typing an address',
  'Улица и дом': 'Street and number',
  'Город': 'City',
  'Индекс': 'Postal code',
  'ПОДТВЕРДИТЬ АДРЕС': 'CONFIRM ADDRESS',
  'Нажмите, чтобы ввести адрес...': 'Tap to enter an address...',
  'Нажмите для поиска адреса...': 'Tap to search address...',
  'Новый клиент': 'New client',
  'СОЗДАТЬ КЛИЕНТА': 'CREATE CLIENT',
  'Название компании (опционально)': 'Company name (optional)',
  'Добавить клиента': 'Add client',
  'Редактировать клиента': 'Edit client',
  'Укажите имя клиента': 'Enter the client name',
  'Список клиентов пуст.\nНажмите (+) чтобы добавить.': 'No clients yet.\nTap (+) to add one.',
  'Поиск по имени, компании или телефону...': 'Search by name, company, or phone...',
  'Сортировка': 'Sort',
  'Имя (А-Я)': 'Name (A–Z)',
  'Сначала новые': 'Newest first',
  'Сначала старые': 'Oldest first',
  'Последние активные': 'Recently active',
  'Без имени': 'No name',
  'История заявок': 'Job history',
  'История связи': 'Calls and messages',
  'Нет звонков и переписки': 'No calls or messages',
  'Исходящий звонок': 'Outgoing call',
  'Письмо от нас': 'Email from us',
  'Письмо': 'Email',
  'SMS от нас': 'SMS from us',
  'Прошлый визит': 'Last visit',
  'Новый ремонт': 'New repair',
  'Позвонить': 'Call',
  'Написать сообщение...': 'Write a message...',
  'Шаблоны': 'Templates',
  'Шаблоны сообщений': 'Message templates',
  'Мои шаблоны': 'My templates',
  'Добавить шаблон': 'Add template',
  'Новый шаблон': 'New template',
  'Изменить шаблон': 'Edit template',
  'Вы хотите подтвердить вызов?': 'Do you want to confirm the visit?',
  'Нет сообщений — напишите первым': 'No messages — write first',
  'Нет сообщений': 'No messages',
  'Переписка с клиентами по SMS\nпоявится здесь':
      'SMS conversations with clients\nwill appear here',
  'Новое сообщение': 'New message',
  'Номер телефона клиента': 'Client phone number',
  'Не удалось отправить сообщение': 'Could not send the message',
  'Не удалось отправить SMS': 'Could not send SMS',
  'SMS отправлено': 'SMS sent',
  'Я в пути': 'On my way',
  'Запчасть заказана': 'Part ordered',
  'Ремонт завершен': 'Repair completed',
  'Набрать номер': 'Dial number',
  'Набор номера': 'Dial pad',
  'Введите номер': 'Enter a number',
  'Цифры': 'Keypad',
  'Микрофон': 'Microphone',
  'Микрофон выкл': 'Mic off',
  'Динамик': 'Speaker',
  'Трубка': 'Earpiece',
  'Соединение...': 'Connecting...',
  'Вызов...': 'Calling...',
  'Звонит': 'Ringing',
  'Входящий звонок': 'Incoming call',
  'Секретарь ИИ': 'AI secretary',
  'Переключить на ИИ': 'Switch to AI',
  'Соединяю с секретарём...': 'Connecting to the secretary...',
  'Звонок завершён': 'Call ended',
  'Звонок пропущен': 'Missed call',
  'Занято': 'Busy',
  'К ОБРАБОТКЕ': 'TO REVIEW',
  'ВСЕ ЗВОНКИ': 'ALL CALLS',
  'Звонки секретаря': 'Secretary calls',
  'Секретарь обрабатывает': 'Secretary is processing',
  'Все звонки обработаны!': 'All calls have been reviewed!',
  'Нет звонков': 'No calls',
  'После звонка ИИ сам расшифрует разговор\nи заявка появится здесь для проверки':
      'After a call, AI will transcribe it\nand the job will appear here for review',
  'ИИ анализирует разговор...': 'AI is analyzing the call...',
  'ИИ обрабатывает': 'AI is processing',
  'Повторить ИИ': 'Retry AI',
  'ИИ запущен повторно': 'AI retry started',
  'Не удалось запустить ИИ': 'Could not start AI',
  'Открыть заявку': 'Open job',
  'Проверьте данные': 'Check the details',
  'Ожидает проверки': 'Needs review',
  'Заявки на проверку': 'Jobs to review',
  'Проверьте заявки от ИИ': 'Review jobs created by AI',
  'Нет заявок, которые нужно проверить': 'No jobs need review',
  'ИИ создал эту заявку после звонка. Проверьте данные и подтвердите.':
      'AI created this job after the call. Check the details and confirm.',
  'ИИ создал эту заявку из письма. Проверьте данные и подтвердите.':
      'AI created this job from an email. Check the details and confirm.',
  'Почта': 'Email',
  'Заявка с телефона': 'Job from a phone call',
  'Заявка с почты': 'Job from email',
  'Склад пуст': 'Warehouse is empty',
  'Склад пуст.': 'Warehouse is empty.',
  'Поиск (Название или Номер)...': 'Search (name or number)...',
  'Новая деталь': 'New part',
  'Редактировать': 'Edit',
  'Название': 'Name',
  'Название запчасти': 'Part name',
  'Категория': 'Category',
  'Модель (Model)': 'Model',
  'Модель / S/N': 'Model / S/N',
  'Цена': 'Price',
  'Цена (\$)': 'Price (\$)',
  'Кол-во': 'Qty',
  'Количество': 'Quantity',
  'Штрихкод (UPC/FNSKU)': 'Barcode (UPC/FNSKU)',
  'УМНЫЙ СКАНЕР ЭТИКЕТКИ': 'SMART LABEL SCANNER',
  'СКАНИРОВАНИЕ...': 'SCANNING...',
  'Сканер этикетки': 'Label scanner',
  'Сканирую…': 'Scanning…',
  'Б/у': 'Used',
  'Взаимозаменяемо': 'Interchangeable',
  'Определяю категорию…': 'Finding the category…',
  'Выбрано автоматически — можно поменять': 'Picked for you — you can change it',
  'на складе': 'in stock',
  'нет в наличии': 'out of stock',
  'Заменяет номера (через запятую)': 'Replaces part numbers (comma separated)',
  'Снять заново': 'Take a new photo',
  'Убрать фото': 'Remove photo',
  'Выбрать картинку': 'Pick a picture',
  'Сделать фото': 'Take photo',
  'Выбрать из галереи': 'Choose from gallery',
  'Текст не найден на фото': 'No text found in the photo',
  'Этикетка успешно отсканирована! ✨': 'Label scanned successfully!',
  'Найдено совпадение': 'Match found',
  'Обновить старую': 'Update existing',
  'Создать новую': 'Create new',
  'Количество добавлено к старой! 📦': 'Quantity added to the existing part',
  'Холодильник': 'Refrigerator',
  'Морозильник': 'Freezer',
  'Стиральная машина': 'Washing machine',
  'Сушилка': 'Dryer',
  'Плита/Духовка': 'Stove/Oven',
  'Посудомойка': 'Dishwasher',
  'Микроволновка': 'Microwave',
  'Универсальное': 'Universal',
  'Запчасть': 'Part',
  'Со склада': 'From warehouse',
  'Запчасть со склада': 'Part from warehouse',
  'Добавить позицию': 'Add line',
  'Добавьте позиции': 'Add line items',
  'Позиции': 'Items',
  'Позиция': 'Item',
  'Счёт': 'Invoice',
  'Смета': 'Estimate',
  'Чек': 'Receipt',
  'Invoice (Счёт)': 'Invoice',
  'Estimate (Смета)': 'Estimate',
  'Создать документ': 'Create document',
  'Отправить счёт': 'Send invoice',
  'Отправить смету': 'Send estimate',
  'Отправить чек': 'Send receipt',
  'Отправить SMS': 'Send SMS',
  'Текст SMS': 'SMS text',
  'Текст SMS пустой': 'SMS text is empty',
  'Подитог': 'Subtotal',
  'Подитог:': 'Subtotal:',
  'Налог': 'Tax',
  'Итого': 'Total',
  'ИТОГО:': 'TOTAL:',
  'Оплачено': 'Paid',
  'Оплачен': 'Paid',
  'К оплате': 'Balance due',
  'Долг': 'Owed',
  'Выставлено': 'Invoiced',
  'Частично': 'Partial',
  'Нет счёта': 'No invoice',
  'Платежи': 'Payments',
  'Принять оплату': 'Take payment',
  'Наличные': 'Cash',
  'Наличные / e-Transfer': 'Cash / e-Transfer',
  'Приложить карту': 'Tap to pay',
  'Приложите карту': 'Present the card',
  'Депозит': 'Deposit',
  'Возврат': 'Refund',
  'Вернуть': 'Refund',
  'Вернуть средства': 'Refund payment',
  'Вернуть средства?': 'Refund payment?',
  'Сумма возврата': 'Refund amount',
  'Максимум': 'Maximum',
  'Частичный возврат': 'Partial refund',
  'Возвращено': 'Refunded',
  'Возврат через Stripe': 'Refunded via Stripe',
  'Частичный возврат Stripe': 'Partial Stripe refund',
  'Укажите сумму возврата': 'Enter a refund amount',
  'Сумма больше оплаченного': 'Amount is more than paid',
  'Возврат выполнен. Работа отменена.':
      'Refund done. Job cancelled.',
  'Stripe вернёт деньги на карту. Наличные отметьте сами из кассы. Полный возврат переведёт работу в «Отмена».':
      'Stripe refunds the card. For cash, take it from the till yourself. A full refund sets the job to Cancelled.',
  'Запросить депозит (Stripe)': 'Request deposit (Stripe)',
  'Выставить инвойс Stripe': 'Create Stripe invoice',
  'Отправить инвойс Stripe': 'Send Stripe invoice',
  'Ссылка скопирована': 'Link copied',
  'Копировать': 'Copy',
  'Печать': 'Print',
  'Нет счетов.\nНажмите (+) и создайте Invoice или Estimate.\nОтправьте из приложения кнопкой «Отправить счёт».':
      'No documents yet.\nTap (+) to create an Invoice or Estimate.\nSend it from the app with Send invoice.',
  'Нет счетов.\nНажмите (+) и создайте Invoice или Estimate.':
      'No invoices.\nTap (+) and create an Invoice or Estimate.',
  'Stripe: ссылка готова': 'Stripe: link ready',
  'Stripe: ссылка отправлена': 'Stripe: link sent',
  'Stripe: оплачен': 'Stripe: paid',
  'Stripe: возврат': 'Stripe: refunded',
  'Stripe: частичный возврат': 'Stripe: partial refund',
  'Stripe (refund)': 'Stripe (refund)',
  'Возврат (наличные)': 'Refund (cash)',
  'SMS со ссылкой отправлено клиенту.': 'SMS with the link was sent to the client.',
  'SMS не ушло. Скопируйте ссылку и отправьте сами.':
      'SMS did not send. Copy the link and send it yourself.',
  'Нет телефона клиента или ссылки': 'No client phone or payment link',
  'Ещё раз по SMS': 'Send SMS again',
  'Оплата': 'Payment',
  'Способ оплаты': 'Payment method',
  'Сумма': 'Amount',
  'Сумма депозита': 'Deposit amount',
  'Документ для оплаты': 'Document to pay',
  'Нет заявок': 'No jobs',
  'Нет активных работ.\nСоздайте их кнопкой ниже.':
      'No active jobs.\nCreate one with the button below.',
  'Нет работ с таким статусом': 'No jobs with this status',
  'Ошибка загрузки списка работ': 'Could not load jobs',
  'Список': 'List',
  'Маршрут': 'Route',
  'Оптимизировать заново': 'Re-optimize',
  'Расставить время по маршруту': 'Schedule times along the route',
  'Время заявок расставлено по маршруту': 'Job times were set along the route',
  'Навигация': 'Navigate',
  'Открыть в Google Maps': 'Open in Google Maps',
  'Вы здесь': 'You are here',
  'Нет запланированных заявок на эту дату\nс указанным адресом.':
      'No scheduled jobs on this date\nwith an address.',
  'Не удалось построить маршрут.': 'Could not build the route.',
  'Проверьте адреса заявок.': 'Check job addresses.',
  'Статус': 'Status',
  'Статус заказа': 'Job status',
  'Проложить маршрут': 'Get directions',
  'Перевести': 'Translate',
  'Выберите статус': 'Choose status',
  'Удалить клиента': 'Delete client',
  'Удалить клиента?': 'Delete this client?',
  'Клиент удалён': 'Client deleted',
  'Клиент в корзине': 'Client moved to trash',
  'Клиенты в корзине': 'Clients moved to trash',
  'Карточка будет удалена. Заявки в календаре останутся.':
      'The client card will be deleted. Jobs in the calendar will stay.',
  'Карточка попадёт в корзину на 30 дней.':
      'The card goes to trash for 30 days.',
  'Карточки попадут в корзину на 30 дней.':
      'The cards go to trash for 30 days.',
  'Выберите приоритет': 'Choose priority',
  'Приоритет': 'Priority',
  '🟢 Обычный': '🟢 Normal',
  '🟡 Средний': '🟡 Medium',
  '🔴 Срочный': '🔴 Urgent',
  'Описание': 'Description',
  'Нажмите, чтобы добавить описание...': 'Tap to add a description...',
  'Решение проблемы': 'Solution',
  'Что сделали, какая деталь, результат...':
      'What you did, which part, the result...',
  'Нужно изменить статус заявки?': 'Do you need to change the job status?',
  'Да, изменить': 'Yes, change it',
  'Изменить статус заявки?': 'Change the job status?',
  'Когда отъедете, спросим статус заявки и SMS следующему клиенту':
      'After you leave we will ask about the job status and texting the next client',
  'Без описания': 'No description',
  'Нет описания': 'No description',
  'Нет описания поломки': 'No problem description',
  'Что взять с собой': 'What to bring',
  'С собой': 'To bring',
  'Фильтр, плата, ключи…': 'Filter, board, keys…',
  'Фото': 'Photos',
  'Смотреть фото': 'View photos',
  'Нет фотографий': 'No photos',
  'Загружается фото…': 'Uploading photo…',
  'Фото загружено': 'Photo uploaded',
  'Нет сети — фото сохранится и загрузится позже': 'No network — the photo will upload later',
  'Ошибка загрузки': 'Upload error',
  'Контакт на месте': 'On-site contact',
  'На объекте': 'On site',
  'Контакт на адресе': 'Contact at the address',
  'Владелец': 'Owner',
  'Арендатор': 'Tenant',
  'Место работы (Job Site)': 'Job site',
  'Не запланировано': 'Not scheduled',
  'Не запланировано — нажмите, чтобы назначить': 'Not scheduled — tap to set a time',
  'Назначить дату и время': 'Set date and time',
  'Изменить дату и время': 'Change date and time',
  'Снять дату': 'Clear date',
  'Дата не указана': 'No date set',
  'Удалить заявку': 'Delete job',
  'Удалить заявку?': 'Delete this job?',
  'Это действие нельзя отменить. Все данные заявки будут удалены.':
      'This cannot be undone. All job data will be deleted.',
  'Заявка попадёт в корзину на 30 дней. Потом удалится навсегда.':
      'The job goes to trash for 30 days, then it is deleted forever.',
  'Заявка удалена': 'Job deleted',
  'Заявка в корзине': 'Job moved to trash',
  'Работа завершена': 'Job completed',
  'Отправить SMS с просьбой об отзыве': 'Send a review request SMS',
  'SMS с просьбой об отзыве отправлено': 'Review request SMS sent',
  'Сначала создайте Invoice на вкладке Финансы': 'First create an Invoice on the Finance tab',
  'ЧАТ': 'CHAT',
  'ДЕТАЛИ': 'DETAILS',
  'ФИНАНСЫ': 'FINANCE',
  'Чат с клиентом': 'Client chat',
  'Кому': 'To',
  'Нет телефона для SMS.\nДобавьте номер клиента или контакта на объекте.':
      'No phone for SMS.\nAdd the client or on-site contact number.',
  'Кому позвонить?': 'Who should I call?',
  'Кому написать?': 'Who should I text?',
  'Мастер': 'Technician',
  'Нет заявок в ожидании запчасти': 'No jobs waiting for a part',
  'Финансовый отчет': 'Financial report',
  'День': 'Day',
  'Неделя': 'Week',
  'Вид': 'View',
  'Месяц': 'Month',
  'Год': 'Year',
  'За выбранный период нет данных.': 'No data for this period.',
  'График выручки': 'Revenue chart',
  'Запчасти (себест.)': 'Parts (cost)',
  'ЧИСТАЯ ПРИБЫЛЬ': 'NET PROFIT',
  'Квартал': 'Quarter',
  'кв.': 'Q',
  'После запчастей': 'After parts',
  'С HST, не для налогов. Для CRA — блок выше.':
      'Includes HST, not for taxes. Use the Canada taxes block above for CRA.',
  'Счета с HST': 'Invoices incl. HST',
  'Получено с HST': 'Collected incl. HST',
  'Выручка без HST': 'Revenue excluding HST',
  'Поиск клиентов, заявок, склада...': 'Search clients, jobs, warehouse...',
  'Введите минимум 2 символа\nдля поиска по клиентам, заявкам и складу':
      'Enter at least 2 characters\nto search clients, jobs, and warehouse',
  'Остаток': 'Stock',
  'Реквизиты компании и HST — в Настройки → Компания и Финансы. Здесь только тексты счетов и смет.':
      'Company details and HST are in Settings → Company and Finance. Here are invoice texts only.',
  'Реквизиты компании и HST — в Настройки → Компания и Финансы. Тексты счетов, смет и SMS клиенту всегда на английском, даже если интерфейс русский.':
      'Company details and HST are in Settings → Company and Finance. Invoice, estimate, and client SMS text stays English even if the app is in Russian.',
  'Эти тексты уходят клиенту и всегда на английском, даже если интерфейс русский.':
      'These texts go to the client and stay English even if the app is in Russian.',
  'Шаблон счёта': 'Invoice template',
  'Шаблон сметы': 'Estimate template',
  'Шаблон после полной оплаты': 'Receipt template after full payment',
  'Смета действует, дней': 'Estimate valid for, days',
  'SMS чека': 'Receipt SMS',
  'Текст в PDF': 'PDF text',
  'Условия счёта': 'Invoice terms',
  'Условия сметы': 'Estimate terms',
  'Подстановки: {name} {company} {phone} {total} {due} {paid} {items} {valid_days} {hst}':
      'Placeholders: {name} {company} {phone} {total} {due} {paid} {items} {valid_days} {hst}',
  'Шаблоны счетов и смет сохранены': 'Invoice and estimate templates saved',
  'Конструктор': 'Builder',
  'Конструктор счёта': 'Invoice builder',
  'Конструктор счёта сохранён': 'Invoice builder saved',
  'Размер логотипа': 'Logo size',
  'Размер QR-кода': 'QR code size',
  'Логотип в шапке': 'Logo in header',
  'Под итогом': 'Under totals',
  'Элементы': 'Elements',
  'Зажмите плитку, чтобы выделить.': 'Long-press a tile to select.',
  'Нажмите «Удалить» сверху или отмените выбор.':
      'Tap Delete at the top or cancel selection.',
  'Зажмите свой статус, чтобы выделить и удалить.':
      'Long-press a custom status to select and delete.',
  'Нажмите «Удалить» внизу или отмените выбор.':
      'Tap Delete at the bottom or cancel selection.',
  'Зажмите или нажмите плитку, чтобы выделить.':
      'Long-press or tap a tile to select.',
  'Базовый статус удалить нельзя': 'Built-in statuses cannot be deleted',
  'Зажмите логотип или QR и перетащите в зону на макете.':
      'Long-press the logo or QR and drag it onto a zone on the layout.',
  'Лого': 'Logo',
  'СОХРАНИТЬ': 'SAVE',
  'Уйдёт SMS из приложения. Текст берётся из Настройки → Счета и сметы.':
      'SMS will be sent from the app. Text comes from Settings → Invoices & estimates.',
  'Просто расскажите о звонке': 'Just tell me about the call',
  'Имя, телефон, адрес, техника, проблема, время визита.\nКак только замолчите — ИИ сам заполнит заявку':
      'Name, phone, address, appliance, problem, visit time.\nWhen you pause, AI will fill in the job',
  'Нажмите на микрофон и расскажите о звонке': 'Tap the microphone and tell me about the call',
  'Слушаю... Говорите о клиенте и проблеме':
      'Listening... Talk about the client and the problem',
  'Говорите — я слушаю и сам всё пойму': 'Speak — I am listening and will fill it in',
  'Обработать введённый текст': 'Process the typed text',
  'Введите или надиктуйте информацию о звонке': 'Type or dictate the call details',
  'Распознавание речи недоступно. Введите текст вручную.':
      'Speech recognition is unavailable. Type the text.',
  'Не услышал речь, слушаю ещё раз...': 'Did not hear speech, listening again...',
  'Не услышал речь. Нажмите на микрофон, чтобы попробовать снова.':
      'Did not hear speech. Tap the microphone to try again.',
  'Анализирую...': 'Analyzing...',
  'ИИ анализирует текст...': 'AI is analyzing the text...',
  'Готово! Можете надиктовать следующий звонок.': 'Done! You can dictate the next call.',
  'Заявка создана!': 'Job created!',
  'Заявка успешно создана!': 'Job created successfully!',
  'Создать клиента и заявку': 'Create client and job',
  'Заполните обязательные поля': 'Fill in the required fields',
  'Обязательное поле': 'Required',
  'Укажите тип техники': 'Enter the appliance type',
  'Укажите телефон клиента': 'Enter the client phone',
  'Укажите адрес клиента': 'Enter the client address',
  'Укажите дату и время визита': 'Enter the visit date and time',
  'Нужны имя или телефон клиента': 'Client name or phone is required',
  'Не удалось создать заявку': 'Could not create the job',
  'Не удалось извлечь данные. Попробуйте добавить больше деталей.':
      'Could not extract the data. Try adding more details.',
  'Нажмите на микрофон и добавьте больше деталей': 'Tap the microphone and add more details',
  'Ошибка обработки. Нажмите на микрофон, чтобы попробовать снова.':
      'Processing error. Tap the microphone to try again.',
  'Ошибка ИИ': 'AI error',
  'Ошибка обработки ИИ': 'AI processing error',
  'Исходный текст': 'Original text',
  'Техника': 'Appliance',
  'Возможные запчасти': 'Possible parts',
  'Добавить еще технику': 'Add another appliance',
  'Аппарат': 'Appliance',
  'Неизвестно': 'Unknown',
  'Новая': 'New',
  'Новая работа': 'New job',
  'Заявка': 'Job',
  'Заявка не найдена': 'Job not found',
  'Документ не найден': 'Document not found',
  'Нет телефона': 'No phone',
  'У клиента нет телефона': 'The client has no phone',
  'У заявки нет телефона': 'The job has no phone',
  'Нет данных для обработки — расшифровка ещё не готова':
      'Nothing to process — the transcript is not ready yet',
  'Обработан': 'Processed',
  'Пропустить': 'Skip',
  'Остановить': 'Stop',
  'Слушать': 'Listen',
  'ЗАКРЫТЬ': 'CLOSE',
  'Нажмите, чтобы выбрать': 'Tap to choose',
  'Нажмите, чтобы добавить': 'Tap to add',
  'По алфавиту': 'A–Z',
  '1 День': '1 day',
  '5 Дней': '5 days',
  '1 день': '1 day',
  '5 дней': '5 days',
  'неделя': 'week',
  'список': 'list',
  'Пн': 'Mon',
  'Вт': 'Tue',
  'Ср': 'Wed',
  'Чт': 'Thu',
  'Пт': 'Fri',
  'Сб': 'Sat',
  'Вс': 'Sun',
  'Обычный': 'Normal',
  'Средний': 'Medium',
  'Срочный': 'Urgent',
  'В пути': 'On the way',
  'Да, отправить': 'Yes, send',
  'Отправить, что вы едете?': 'Send that you are on the way?',
  'SMS «я в пути» отправлено': 'On-the-way SMS sent',
  'Не нашёл, кому отправлять SMS': 'Could not find who to text',
  'Это время пересекается с другой заявкой': 'This time overlaps another job',
  'Не удалось перенести заявку': 'Could not reschedule the job',
  'Ошибка загрузки настроек': 'Could not load settings',
  'Список адресов пуст': 'Address list is empty',
  'Ошибка Autocomplete': 'Autocomplete error',
  'Нет доступа к микрофону': 'No microphone access',
  'Не настроен ключ Gemini': 'Gemini key is not set',
  'Подключаюсь...': 'Connecting...',
  'Не удалось подключиться': 'Could not connect',
  'Ошибка связи': 'Connection error',
  'Переподключаюсь...': 'Reconnecting...',
  'Связь оборвалась': 'Connection lost',
  'Слушаю — нажмите для паузы': 'Listening — tap to pause',
  'Пауза': 'Paused',
  'Пауза — нажмите, чтобы слушать': 'Paused — tap to listen',
  'Фикс отвечает': 'Assistant is speaking',
  'Нажмите кружок — пауза. В сторону — закрыть.':
      'Tap the circle to pause. Tap outside to close.',
  'Звонки Twilio, шапка SMS и шаблоны': 'Twilio calls, SMS header and templates',
  'HST / GST, подпись, счета и сметы': 'HST / GST, signature, invoices',
  'No tax 0%': 'No tax 0%',
  'Остаток:': 'Stock:',
  'Не удалось соединиться': 'Could not connect',
  'Завершён': 'Completed',
  'Без ответа': 'No answer',
  'Не удался': 'Failed',
  'В процессе': 'In progress',
  'Подготовка микрофона...': 'Preparing microphone...',
  'Останавливаю...': 'Stopping...',
  'Хотите, я отправлю следующему клиенту уведомление о том, что вы едете?':
      'Want me to text the next client that you are on the way?',
  'Предложение отправить SMS следующему клиенту': 'Prompt to send SMS to the next client',
  'Предварительный расчёт': 'Estimate',
  'Ссылка Stripe (если клиента нет рядом)': 'Stripe link (if the client is not here)',
  'Отправить ссылку': 'Send link',
  'Неоплачен': 'Unpaid',
  'остаток': 'in stock',
  'шт': 'pcs',
  'Подтвердить': 'Confirm',
  'Продолжить': 'Continue',
  'Отменить': 'Cancel',
  'На складе уже есть эта деталь': 'This part is already in the warehouse',
  'Такой номер уже есть на складе': 'This part number is already in stock',
  'Такой номер уже есть — в корзине': 'This part number is in the trash',
  'Сверьте карточки. Объединить — одна деталь. Оставить две — обе.':
      'Compare the cards. Merge keeps one. Keep two leaves both.',
  'В корзине': 'In trash',
  'Вы вводите': 'You are adding',
  'Оставить две': 'Keep two',
  'Объединить': 'Merge',
  'Карточки объединены': 'Cards merged',
  'Вернул из корзины и объединил': 'Restored from trash and merged',
  'Этой детали нет — есть замена': 'This number is not in stock — a substitute is',
  'Можно прибавить количество к той карточке или оставить две.':
      'Add the quantity to that card, or keep two cards.',
  'ИИ ищет замены…': 'AI is looking up replacements…',
  'ИИ подставил номера замен': 'AI filled replacement numbers',
  'Добавил к замене на складе': 'Added to the substitute in stock',
  'тот же номер под другим артикулом': 'same part, different number',
  'ваша пометка': 'your note',
  'подсказал ИИ': 'AI suggestion',
  'Не удалось сохранить деталь': 'Could not save the part',
  'Запчасть с номером': 'A part with number',
  'уже есть на складе. Что вы хотите сделать?':
      'is already in the warehouse. What do you want to do?',
  'Визит запланирован на': 'Visit scheduled for',
  'Заявка перенесена на': 'Job moved to',
  'Делаю': 'Working',
  'Точек': 'Points',
  'точек': 'stops',
  'км': 'km',
  'ч': 'h',
  'мин': 'min',
  'мин по умолчанию': 'min default',
  'мин буфер в календаре': 'min buffer in calendar',
  'Завершено работ': 'Jobs completed',
  'Новые заявки': 'New jobs',
  'Собрано': 'Collected',
  'На счёте Stripe': 'Stripe balance',
  'Ожидает Stripe': 'Stripe pending',
  'Новое SMS': 'New SMS',
  'Счёт на оплату': 'Invoice',
  'Оплатить': 'Pay',
  'Оплачено картой': 'Paid by card',
  'Платёж не завершён': 'Payment was not completed',
  'Попробуйте ещё раз': 'Please try again',
  'Заявки на день': 'Jobs for today',
  'В 7:00 список заявок и что взять с собой': 'At 7:00, the job list and what to bring',
  'Взять': 'Bring',
  'заявка': 'job',
  'заявки': 'jobs',
  'заявок': 'jobs',
  'Ошибка сканера': 'Scanner error',
  'Впишите название детали': 'Type the part name',
  'Фото не загрузилось — нет связи. Деталь сохранится без него.':
      'The photo did not upload — no connection. The part will be saved without it.',
  'Ошибка Stripe': 'Stripe error',
  'Ошибка терминала': 'Terminal error',
  'Ошибка сохранения': 'Save error',
  'Ошибка распознавания': 'Recognition error',
  'Не удалось начать слушать': 'Could not start listening',
  'Новое': 'New',
  'в': 'at',
  'Email': 'Email',
  'SMS': 'SMS',
  'Пример: Звонил Иван Петров, телефон 416-555-1234, живёт на 123 Main Street, Торонто. У него сломалась стиральная машина Samsung, не сливает воду. Договорились на завтра в 10 утра.':
      'Example: Ivan Petrov called, phone 416-555-1234, lives at 123 Main Street, Toronto. His Samsung washer is not draining. Booked for tomorrow at 10 a.m.',
  'Другое': 'Other',
  'Категория техники': 'Appliance category',
  'Поиск': 'Search',
  'Фильтр': 'Filter',
  'Нет заявок в этой группе': 'No jobs in this group',
  'Клиенту уйдёт SMS на английском. Нажмите «Отправить» и выберите файл или ссылку.':
      'The client will get an English SMS. Tap Send and choose a file or a link.',
  'Как отправить?': 'How to send?',
  'Ссылка': 'Link',
  'PDF уйдёт вложением в SMS.': 'The PDF will be attached to the SMS.',
  'Клиент получит ссылку, чтобы скачать PDF.':
      'The client will get a link to download the PDF.',
  'Сбросить': 'Reset',
  'Закрыть': 'Close',
  'Нет данных': 'No data',
  'Да': 'Yes',
  'Выбрать': 'Select',
  'Изменить': 'Edit',
  'Копировать ссылку': 'Copy link',
  'Открыть ссылку': 'Open link',
  'Оплата картой': 'Card payment',
  'Терминал': 'Terminal',
  'Нет позиций': 'No line items',
  'Скидка': 'Discount',
  'Чаевые': 'Tip',
  'Корзина': 'Trash',
  'в корзине — без даты визита': 'in the basket — no visit date',
  'без даты визита': 'unscheduled',
  'Без даты визита': 'Unscheduled',
  'Клиенты без заявки': 'Clients without a job',
  'Комментарий': 'Comment',
  'Заметки': 'Notes',
  'История': 'History',
  'Активные': 'Active',
  'Нет активных заявок': 'No active jobs',
  'Архив': 'Archive',
  'Карта': 'Map',
  'Клиенты на карте': 'Clients on the map',
  'Нет клиентов с адресом': 'No clients with an address',
  'Не удалось отметить часть адресов': 'Some addresses could not be placed',
  'Клиент с этим номером уже есть': 'A client with this number already exists',
  'Клиенты с этим номером уже есть': 'Clients with this number already exist',
  'Открыть карточку?': 'Open the card?',
  'Открыть клиента': 'Open client',
  'Список заявок': 'Job list',
  'Нет маршрута': 'No route',
  'Построить маршрут': 'Build route',
  'Моё местоположение': 'My location',
  'Клиент не найден': 'Client not found',
  'Сохранён': 'Saved',
  'Сохранено': 'Saved',
  'Изменения сохранены': 'Changes saved',
  'Удалено': 'Deleted',
  'Клиент сохранён': 'Client saved',
  'Заявка сохранена': 'Job saved',
  'Документ сохранён': 'Document saved',
  'Документ удалён': 'Document deleted',
  'Позиция добавлена': 'Line added',
  'Позиция удалена': 'Line removed',
  'Оплата сохранена': 'Payment saved',
  'Нет доступа': 'No access',
  'Разрешите доступ к камере': 'Allow camera access',
  'Разрешите доступ к геолокации': 'Allow location access',
  'Разрешите доступ к микрофону': 'Allow microphone access',
  'Нет интернета': 'No internet',
  'Повторите попытку': 'Try again',
  'Что-то пошло не так': 'Something went wrong',
  'Обязательно': 'Required',
  'Неверный номер': 'Invalid number',
  'Неверный email': 'Invalid email',
  'Выберите клиента': 'Choose a client',
  'Выберите тип': 'Choose a type',
  'Добавить технику': 'Add appliance',
  'Удалить технику': 'Remove appliance',
  'Модель': 'Model',
  'Серийный номер': 'Serial number',
  'Проблема': 'Problem',
  'Комментарий мастера': 'Technician notes',
  'Внутренние заметки': 'Internal notes',
  'Фото до': 'Photos before',
  'Фото после': 'Photos after',
  'Галерея': 'Gallery',
  'Камера': 'Camera',
  'Удалить фото?': 'Delete photo?',
  'Подпись клиента': 'Client signature',
  'Подписано': 'Signed',
  'Не подписано': 'Not signed',
  'Очистить подпись': 'Clear signature',
  'Отправить документ': 'Send document',
  'Скачать PDF': 'Download PDF',
  'Поделиться': 'Share',
  'Нет чата': 'No chat',
  'Введите сообщение': 'Enter a message',
  'Отправить': 'Send',
  'Доставлено': 'Delivered',
  'Прочитано': 'Read',
  'Не отправлено': 'Not sent',
  'Шаблон': 'Template',
  'Выберите шаблон': 'Choose a template',
  'Входящие': 'Incoming',
  'Исходящие': 'Outgoing',
  'Пропущенные': 'Missed',
  'Длительность': 'Duration',
  'Запись': 'Recording',
  'Расшифровка': 'Transcript',
  'Нет расшифровки': 'No transcript',
  'Создать из звонка': 'Create from call',
  'Принято': 'Accepted',
  'Отклонено': 'Declined',
  'Сбросить фильтр': 'Clear filter',
  'Сортировать': 'Sort',
  'По дате': 'By date',
  'По имени': 'By name',
  'По статусу': 'By status',
  'Сегодня нет заявок': 'No jobs today',
  'Нет заявок на этот день': 'No jobs on this day',
  'Предыдущий день': 'Previous day',
  'Следующий день': 'Next day',
  'На эту неделю нет заявок': 'No jobs this week',
  'Выручка': 'Revenue',
  'Расходы': 'Expenses',
  'Прибыль': 'Profit',
  'Средний чек': 'Average ticket',
  'Оплачено за период': 'Paid this period',
  'Долг за период': 'Owed this period',
  'Нет запчастей в очереди': 'No parts in the queue',
  'Ожидают запчасть': 'Waiting for a part',
  'Заказать': 'Order',
  'Заказано': 'Ordered',
  'Получено': 'Received',
  'На складе': 'In stock',
  'Мало': 'Low',
  'Нет в наличии': 'Out of stock',
  'Ед.': 'Units',
  'Артикул': 'SKU',
  'Поставщик': 'Supplier',
  'Минимум': 'Minimum',
  'Добавить на склад': 'Add to warehouse',
  'Списать': 'Write off',
  'Оприходовать': 'Receive stock',
  'Поиск по складу': 'Search warehouse',
  'Деталь сохранена': 'Part saved',
  'Деталь удалена': 'Part deleted',
  'Удалить деталь?': 'Delete this part?',
  'Этот элемент будет удалён со склада.': 'This item will be removed from the warehouse.',
  'Выберите запчасть': 'Choose a part',
  'Нет подходящих запчастей': 'No matching parts',
  'Добавить вручную': 'Add manually',
  'Количество на складе': 'Quantity in stock',
  'Цена продажи': 'Sale price',
  'Себестоимость': 'Cost',
  'НДС': 'Tax',
  'Без названия': 'Untitled',
  'Без адреса': 'No address',
  'Без телефона': 'No phone',
  'Без компании': 'No company',
  'Основной': 'Primary',
  'Дополнительный': 'Additional',
  'Контакты': 'Contacts',
  'Добавить контакт': 'Add contact',
  'Редактировать контакт': 'Edit contact',
  'Удалить контакт?': 'Delete contact?',
  'Заявки клиента': 'Client jobs',
  'Нет заявок у этого клиента': 'This client has no jobs',
  'Последняя заявка': 'Last job',
  'Всего заявок': 'Total jobs',
  'Баланс': 'Balance',
  'Перейти в чат': 'Open chat',
  'Маршрут до клиента': 'Route to client',
  'Скопирован': 'Copied',
  'Телефон скопирован': 'Phone copied',
  'Адрес скопирован': 'Address copied',
  'Не удалось позвонить': 'Could not place the call',
  'Не удалось открыть карту': 'Could not open the map',
  'Разрешения': 'Permissions',
  'Удалить значение?': 'Delete this value?',
  'Дата': 'Date',
  'Время': 'Time',
  'Зона:': 'Zone:',
  'Имя:': 'Name:',
  'Клиент:': 'Client:',
  'Приоритет:': 'Priority:',
  'Бренд:': 'Brand:',
  'Модель:': 'Model:',
  'Сумма:': 'Amount:',
  'Налог (': 'Tax (',
  'Маршрут:': 'Route:',
  'Маршрут (': 'Route (',
  'SMS отправлено клиенту': 'SMS sent to the client',
  'Включите геолокацию в настройках телефона': 'Turn on location in phone settings',
  'Нет доступа к геолокации. Разрешите её для приложения.':
      'No location access. Allow it for the app.',
  'Геолокация запрещена. Откройте настройки приложения и включите доступ к месту.':
      'Location is denied. Open app settings and allow location access.',
  'Не удалось получить координаты. Повторите на открытом месте.':
      'Could not get coordinates. Try again outdoors.',
  'Не удалось получить координаты. Проверьте GPS.': 'Could not get coordinates. Check GPS.',
  'Не задан Google API ключ': 'Google API key is not set',
  'Google не вернул ни одного маршрута': 'Google did not return a route',
  'Когда отъедете на 2 км, предложим SMS следующему клиенту':
      'After you drive 2 km away, we will offer an SMS to the next client',
  'Разрешаю геолокацию для терминала...': 'Allowing location for the terminal...',
  'Подключаю терминал Stripe...': 'Connecting Stripe terminal...',
  'Тестовый режим: сейчас будет симулятор карты': 'Test mode: a card simulator will appear',
  'Ищу NFC на этом телефоне...': 'Looking for NFC on this phone...',
  'Создаю платёж...': 'Creating the payment...',
  'В симуляторе нажмите, что карта приложена':
      'In the simulator, tap that the card was presented',
  'Поднесите карту клиента к задней панели телефона':
      'Hold the client card to the back of the phone',
  'Сохраняю оплату в заявку...': 'Saving the payment to the job...',
  'Готовлю приём карты...': 'Preparing card payment...',
  'Не удалось запустить тестовый терминал. Проверьте интернет.':
      'Could not start the test terminal. Check the internet.',
  'Телефон не увидел NFC. Включите NFC в настройках Android.':
      'The phone did not see NFC. Turn on NFC in Android settings.',
  'Оплата отменена': 'Payment cancelled',
  'Включите NFC в настройках телефона и попробуйте снова.':
      'Turn on NFC in phone settings and try again.',
  'Этот телефон не поддерживает приём карты через NFC.':
      'This phone cannot accept cards over NFC.',
  'Настоящая карта на debug-сборке не принимается. Нужна release-сборка и живые ключи Stripe.':
      'A real card is not accepted on a debug build. You need a release build and live Stripe keys.',
  'Tap to Pay не работает, пока включены параметры разработчика. Выключите их для живых платежей.':
      'Tap to Pay does not work while developer options are on. Turn them off for live payments.',
  'Телефон не проходит проверку безопасности Stripe для Tap to Pay.':
      'This phone fails Stripe security checks for Tap to Pay.',
  'сегодня': 'today',
  'вчера': 'yesterday',
  'Не удалось определить ваше местоположение.': 'Could not determine your location.',
  'Разрешите доступ к геолокации в настройках телефона.':
      'Allow location access in phone settings.',
  'Попробовать снова': 'Try again',
  'Добавлено в заявку': 'Added to the job',
  'Фото или вложение': 'Photo or attachment',
  'Русский': 'Russian',
  'Отменён': 'Cancelled',
  'Оплачено через Stripe': 'Paid via Stripe',
  'Ссылка Stripe активна': 'Stripe link is active',
  'Определяем по GPS…': 'Detecting via GPS…',
  'дн. назад': 'd ago',
  'нед. назад': 'wk ago',
  'мес. назад': 'mo ago',
  'заявок без адреса не включены в маршрут':
      'jobs without an address were left out of the route',
  'Хотите отправить': 'Want to send',
  'уведомление, что вы в пути?': 'a notification that you are on the way?',
  'Приём карты на телефоне сейчас работает на Android (Samsung).':
      'Card payments on the phone currently work on Android (Samsung).',
  'Для приёма карты нужна геолокация (требование Stripe Terminal).':
      'Card payments require location (a Stripe Terminal requirement).',
  'История визитов': 'Visit history',
  'Добавить визит': 'Add visit',
  'Удалить визит': 'Delete visit',
  'Визит': 'Visit',
  'визитов': 'visits',
  'Запланирован': 'Scheduled',
  'Выполнен': 'Done',
  'Длительность нового визита': 'Default visit length',
  'Заметка': 'Note',
  'Например: диагностика, установка': 'For example: diagnosis, install',
  'Диагностика': 'Diagnosis',
  'Установка': 'Install',
  'Повторный выезд': 'Return visit',
  'Повторный визит': 'Repeat visit',
  'След. визит': 'Next visit',
  'Не запланировано — нажмите «Добавить», чтобы назначить':
      'Not scheduled — tap Add to set a visit',
  'Ожидание запчасти — дату возврата ставить не нужно. Добавьте визит, когда запчасть приедет.':
      'Waiting for a part — no return date needed. Add a visit when the part arrives.',
  'Дата следующего визита не нужна, пока нет запчасти. Когда она приедет — добавьте визит. Заявка в очереди запчастей (фургон).':
      'No next visit date is needed until the part arrives. Then add a visit. The job is in the parts queue (van).',
  'Электронный адрес': 'Email',
  'Откуда узнали': 'How they found us',
  'Добавить владельца': 'Add owner',
  'Добавить другое место работы': 'Add another job site',
  'Другое место работы': 'Job site',
  'CSV-шаблон для импорта': 'CSV import template',
  'Шаблон готов': 'Template is ready',
  'Название, цвет, свои статусы': 'Name, color, custom statuses',
  'Нажмите статус, чтобы сменить название и цвет. Плюс в списке добавляет новый.':
      'Tap a status to change its name and color. The plus in the list adds a new one.',
  'Добавлено': 'Added',
  'Укажите имя и телефон': 'Enter name and phone',
  'Экран и шрифт': 'Display & font',
  'Размер экрана': 'Screen size',
  'Увеличивает текст и элементы на всех экранах.':
      'Makes text and controls larger or smaller on every screen.',
  'Увеличивает текст, нижнюю панель и элементы на всех экранах. Цвета шапки, меню и нижней полоски тоже меняются здесь.':
      'Enlarges text, the bottom bar, and controls on every screen. Header, menu, and system bar colors also change here.',
  'Шрифт': 'Font',
  'По умолчанию': 'Default',
  'Мельче': 'Smaller',
  'Крупнее': 'Larger',
  'Крупный': 'Largest',
  'Статусы заявок': 'Job statuses',
  'Можно добавить свои. Базовые нельзя удалить.':
      'You can add your own. Built-in statuses cannot be deleted.',
  'Источники для карточки клиента': 'Sources for the client card',
  'Базовый': 'Built-in',
  'Изменить статус': 'Edit status',
  'Новый статус': 'New status',
  'Выберите оттенок': 'Pick a shade',
  'Синий': 'Blue',
  'Индиго': 'Indigo',
  'Фиолетовый': 'Purple',
  'Жёлтый': 'Yellow',
  'Оранжевый': 'Orange',
  'Красный': 'Red',
  'Серый': 'Gray',
  'Голубой': 'Cyan',
  'Базовые статусы нужны программе и их нельзя удалить. Свои можно добавить и переименовать.':
      'Built-in statuses are required. You can add and rename your own.',
  'Без даты': 'No date',
  'Время не указано': 'No time set',
  'Без шапки — только текст сообщения': 'No header — message text only',
  'Только эта строка ставится в начало SMS. Название компании туда не добавляется. Пример: www.FIX-Appliance.CA. Оставьте пустым, если шапка не нужна.':
      'Only this line is placed at the start of SMS. The company name is not added. Example: www.FIX-Appliance.CA. Leave empty for no header.',
  'Размер элементов и шрифт': 'Element size and font',
  'Техника, бренды, статусы, источники': 'Appliances, brands, statuses, sources',
  'Рекомендация': 'Referral',
  'Пишите по-русски — клиенту уйдёт на английском':
      'Write in Russian — the client gets English',
  'Отправлено на английском': 'Sent in English',
  'Оригинал': 'Original',
  'Перевод...': 'Translating...',
  'Повтор': 'Callback',
  'Повторный вызов': 'Repeat call',
  'Повторный клиент': 'Repeat client',
  'Исходная заявка': 'Original job',
  'Листовка': 'Flyer',
  'Моноширинный': 'Monospace',
  'Изменить адрес': 'Edit address',
  'Ожидаемые запчасти': 'Expected parts',
  'Запчасть не указана': 'Part not specified',
  'Импорт и экспорт': 'Import & export',
  'Клиенты, заявки, CSV и бэкап': 'Clients, jobs, CSV and backup',
  'Экспорт клиентов': 'Export clients',
  'Экспорт заявок': 'Export jobs',
  'Полный бэкап': 'Full backup',
  'Импорт из файла': 'Import from file',
  'CSV для Excel': 'CSV for Excel',
  'JSON клиентов и заявок': 'JSON of clients and jobs',
  'Файл клиентов готов': 'Clients file is ready',
  'Файл заявок готов': 'Jobs file is ready',
  'Бэкап готов': 'Backup is ready',
  'Импорт клиентов': 'Import clients',
  'Найти совпадения': 'Find matches',
  'Выбрать все': 'Select all',
  'выбрано': 'selected',
  'Удалить клиентов?': 'Delete clients?',
  'Карточки будут удалены. Заявки в календаре останутся.':
      'Cards will be deleted. Jobs on the calendar will stay.',
  'Клиенты удалены': 'Clients deleted',
  'Нет совпадений': 'No matches',
  'Совпадения': 'Matches',
  'Оставить этого': 'Keep this one',
  'Совпадение': 'Match',
  'Имя и адрес': 'Name and address',
  'Вставьте номер': 'Paste number',
  'Зажмите, чтобы вставить': 'Long-press to paste',
  'В буфере нет номера': 'No number on the clipboard',
  'Номер вставлен': 'Number pasted',
  'Вставить номер': 'Paste number',
  'Отслеживание': 'Tracking',
  'Трек-номер': 'Tracking number',
  'Номер заказа Amazon': 'Amazon order number',
  'Номер заказа': 'Order number',
  'Другой': 'Other',
  'Или оставьте сумму ремонта: на экране карты клиент сам добавит чаевые.':
      'Or leave the repair total: the customer can add a tip on the card screen.',
  'Номер Amazon / трек — нажмите': 'Amazon / tracking — tap to add',
  'Курьер сегодня': 'Out for delivery',
  'Отправлено': 'Shipped',
  'Выше': 'Move up',
  'Ниже': 'Move down',
  'Стрелки или зажмите карточку, чтобы поменять местами':
      'Use arrows or long-press a card to reorder',
  'Стрелки или зажмите карточку, чтобы поменять порядок остановок':
      'Use arrows or long-press a card to reorder stops',
  'Прикрепить': 'Attach',
  'Файл': 'File',
  'SMS при записи': 'Booking SMS',
  'Подтверждение даты и ответ 1 / 2': 'Confirms the date; client replies 1 / 2',
  'После вашего подтверждения заявки. Клиент отвечает 1 / 0 / 5':
      'After you confirm the job. The client replies 1 / 0 / 5',
  'Напоминание за сутки': 'Day-before reminder',
  'Авто-SMS накануне визита': 'Automatic SMS the day before the visit',
  'Авто-отзыв после работы': 'Auto review after the job',
  'Предлагать отзыв после работы': 'Offer review after the job',
  'SMS со ссылкой на Google после «Завершено»':
      'SMS with a Google review link after Completed',
  'При завершении и после оплаты спросить — само не шлёт':
      'Ask on complete and after payment — never send by itself',
  'Шаблон «Запись на визит»': 'Booking confirmation template',
  'Шаблон «Напоминание за сутки»': 'Day-before template',
  'Ссылка на Google-отзыв': 'Google review link',
  '{review} в шаблоне «Ремонт завершен»':
      '{review} in the “Job completed” template',
  '{name} {date} {time} {address} {review}':
      '{name} {date} {time} {address} {review}',
  'Клиент подтвердил': 'Client confirmed',
  'Просит перенос': 'Asked to reschedule',
  'Ждём SMS': 'Waiting for SMS',
  'Нужна подпись клиента': 'Client signature is required',
  'Эта техника раньше': 'This appliance before',
  'S/N': 'S/N',
  'Прайсбук': 'Pricebook',
  'Good / Better / Best для смет': 'Good / Better / Best for quotes',
  'Три цены на типовую работу. В смете мастер выбирает Good / Better / Best.':
      'Three prices per typical job. On a quote the tech picks Good / Better / Best.',
  'Добавить работу': 'Add a job',
  'Типовая работа': 'Standard job',
  'PDF сметы': 'Estimate PDF',
  'PDF чека': 'Receipt PDF',
  'Оплатить картой': 'Pay by card',
  'Отправить ссылку на оплату': 'Send payment link',
  'E-перевод': 'E-transfer',
  'Спасибо, оплата прошла успешно': 'Thank you, payment successful',
  'Оплата не прошла': 'Payment failed',
  'Все SMS клиенту в одном месте, всегда English':
      'All client SMS in one place, always English',
  'Телефон или терминал Stripe': 'Phone or Stripe terminal',
  'Все карточные платежи идут только через Stripe. Можно принять карту на телефоне или на терминале Stripe, который вы покупаете в Dashboard Stripe.':
      'All card payments go through Stripe only. Take the card on the phone or on a Stripe terminal bought from the Stripe Dashboard.',
  'Телефон · Stripe Tap to Pay': 'Phone · Stripe Tap to Pay',
  'Клиент прикладывает карту к задней панели':
      'Customer taps the card on the back of the phone',
  'Терминал Stripe': 'Stripe terminal',
  'WisePad 3 или другой ридер из Stripe Dashboard. Карта идёт в устройство, деньги — в Stripe.':
      'WisePad 3 or another reader from the Stripe Dashboard. The card goes in the device, money goes to Stripe.',
  'Stripe Reader M2. Карта идёт в ридер, деньги — в Stripe.':
      'Stripe Reader M2. The card goes in the reader, money goes to Stripe.',
  'BBPOS WisePad 3. Карта вставляется или прикладывается, деньги — в Stripe.':
      'BBPOS WisePad 3. Insert or tap the card; money goes to Stripe.',
  'Купите ридер в Stripe (Dashboard → Terminal). Включите Bluetooth и сам терминал, затем нажмите «Найти терминал».':
      'Buy the reader in Stripe (Dashboard → Terminal). Turn on Bluetooth and the reader, then tap Find terminal.',
  'Включите Bluetooth на телефоне и Reader M2, затем нажмите «Найти терминал».':
      'Turn on Bluetooth on the phone and the Reader M2, then tap Find terminal.',
  'Включите Bluetooth на телефоне и WisePad 3, затем нажмите «Найти терминал».':
      'Turn on Bluetooth on the phone and the WisePad 3, then tap Find terminal.',
  'Найти терминал': 'Find terminal',
  'Ищу терминал…': 'Looking for the terminal…',
  'Купить терминал в Stripe': 'Buy a terminal in Stripe',
  'Терминал подключён': 'Terminal connected',
  'Вставьте, приложите или проведите карту на терминале Stripe':
      'Insert, tap, or swipe the card on the Stripe terminal',
  'Ищу терминал Stripe по Bluetooth. Включите его.':
      'Looking for the Stripe terminal over Bluetooth. Turn it on.',
  'Ищу тестовый терминал Stripe...': 'Looking for the test Stripe terminal...',
  'Терминал не найден. Включите его, поднесите ближе и купите ридер в Stripe Dashboard → Terminal.':
      'Terminal not found. Turn it on, bring it closer, and buy a reader in Stripe Dashboard → Terminal.',
  'Для терминала Stripe включите Bluetooth.':
      'Turn on Bluetooth for the Stripe terminal.',
  'Терминал Stripe сейчас подключается на Android.':
      'The Stripe terminal currently pairs on Android.',
  'На терминале': 'On the terminal',
  'Ожидаю карту на терминале': 'Waiting for the card on the terminal',
  'Обновляю прошивку терминала...': 'Updating the terminal firmware...',
  'Все SMS клиенту, всегда English': 'All client SMS, always English',
  'Логотип на инвойсе': 'Logo on invoice',
  'QR-код внизу': 'QR code at the bottom',
  'История платежей на PDF': 'Payment history on the PDF',
  'Вид PDF, условия, превью инвойса': 'PDF look, terms, invoice preview',
  'Прайсбук пуст — добавьте работы в Каталоге':
      'Pricebook is empty — add jobs in Catalog',
  'Наценка на запчасти': 'Parts markup',
  'Берётся цена со склада': 'Uses the warehouse sell price',
  'Серийник и модель появятся после заявок':
      'Serial and model will appear after jobs',
  'Уменьшить': 'Zoom out',
  'Приблизить': 'Zoom in',
  'Каталог типичных работ с тремя ценами: Good / Better / Best. Это не запчасть со склада.':
      'Catalog of typical jobs with three prices: Good / Better / Best. This is not a warehouse part.',
  'Реквизиты компании и HST — в Настройки → Компания и Финансы. PDF клиенту всегда на английском.':
      'Company details and HST are in Settings → Company and Finance. Client PDFs stay in English.',
  'SMS счёта, сметы и ссылки на оплату':
      'Invoice, estimate, and payment-link SMS',
  'В SMS ссылку нельзя спрятать за кнопку — она идёт отдельной строкой Pay here.':
      'SMS cannot hide a URL behind a button — it stays on its own Pay here line.',
  'Запись разговора': 'Call recording',
  'Прослушать запись': 'Play recording',
  'Выжимка': 'Summary',
  'Разговор на русском': 'Conversation in Russian',
  'Разговор': 'Conversation',
  'Пока нет выжимки': 'No summary yet',
  'Пока нет расшифровки': 'No transcript yet',
  'Перевожу...': 'Translating...',
  'Перевожу разговор...': 'Translating the conversation...',
  'Не удалось загрузить запись': 'Could not load the recording',
  'Готовим запись...': 'Preparing the recording...',
  'Не удалось включить запись': 'Could not play the recording',
  'После разговора появится запись': 'The recording will appear after the call',
  'Зажмите карточку, чтобы поменять порядок остановок':
      'Long-press a card to reorder stops',
  'Звонок': 'Call',
  'Подтверждение заказа': 'Order confirmation',
  'Заказ принят': 'Order accepted',
  'Заказ не принят': 'Order not accepted',
  'На рассмотрении': 'Under review',
  'Вибрация при нажатии': 'Vibration on tap',
  'Отклик на кнопки, вкладки и свайпы':
      'Feedback on buttons, tabs, and swipes',
  'Цвета приложения': 'App colors',
  'Готовые темы': 'Ready-made themes',
  'Синий и жёлтый': 'Navy and yellow',
  'Зелёный': 'Green',
  'Бордо': 'Wine',
  'Графит': 'Graphite',
  'Бирюза': 'Teal',
  'Ночь': 'Night',
  'Основной цвет': 'Main color',
  'Шапка, меню, кнопки': 'Header, menu, buttons',
  'Жёлтые кнопки, выделения': 'Yellow buttons and highlights',
  'Опасный': 'Danger',
  'Отмена и ошибки': 'Cancel and errors',
  'Рабочая карточка': 'Job card',
  'Бумага карточки': 'Card paper',
  'Старая книга': 'Old book',
  'Слоновая кость': 'Ivory',
  'Кремовая': 'Cream',
  'Крафт': 'Kraft',
  'Имя на карточке': 'Name on the card',
  'Адрес на карточке': 'Address on the card',
  'Обычный текст': 'Body text',
  'Размер имени': 'Name size',
  'Вид текста': 'Text style',
  'Системный': 'System',
  'Книжный': 'Book',
  'Акцент': 'Accent',
  'Поиск по любой информации в карточке...': 'Search any info on the client card...',
  'Найти в карточке...': 'Find on the card...',
  'Напоминание о визите': 'Visit reminder',
  'Авто-SMS клиенту перед визитом': 'Auto SMS to the client before the visit',
  'Выберите, когда отправлять': 'Choose when to send',
  'За 2 дня': '2 days before',
  'За сутки': '1 day before',
  'Утром в день визита': 'Morning of the visit',
  'За 2 часа': '2 hours before',
  'Когда слать напоминание': 'When to send the reminder',
  'Можно выбрать несколько раз. Клиенту уходит English SMS.':
      'You can pick several times. The client always gets English SMS.',
  'Время утреннего SMS': 'Morning SMS time',
  'Утреннее SMS': 'Morning SMS',
  'Утреннее уведомление': 'Morning notification',
  'Вечернее уведомление': 'Evening notification',
  'Расстояние, км': 'Distance, km',
  'Спросить после этого отъезда от клиента':
      'Ask after leaving the client by this distance',
  'После': 'After',
  'Утром': 'Morning',
  'Вечером': 'Evening',
  'вечером': 'evening',
  'Клиент подтвердил ремонт': 'Client confirmed the repair',
  'Ждём согласие клиента': 'Waiting for the client to agree',
  'Сделать счёт': 'Make invoice',
  'Счёт уже создан из этой сметы': 'An invoice was already made from this estimate',
  'Клиенту уйдёт SMS со ссылкой. Он откроет её и подтвердит ремонт. PDF не отправляем.':
      'The client gets an SMS with a link. They open it and confirm the repair. We do not send a PDF.',
  'Включите Геолокацию в шторке телефона. Без неё Android не показывает Bluetooth-терминал.':
      'Turn on Location in the Android shade. Without it Android will not show a Bluetooth terminal.',
  'Ищу терминал. Bluetooth должен быть включён, сам ридер — тоже. Для WisePad 3 подержите кнопку питания, пока не замигает.':
      'Looking for the terminal. Bluetooth and the reader must be on. For WisePad 3, hold the power button until it blinks.',
  'По Bluetooth не нашёл. Ищу Wi‑Fi / Ethernet терминал в Stripe...':
      'Nothing on Bluetooth. Looking for a Wi-Fi / Ethernet terminal in Stripe...',
  'Терминал не найден. Включите его, Геолокацию и Bluetooth. WisePad 3 / M2 — держите рядом. S700 должен быть онлайн в Stripe Dashboard → Terminal.':
      'Terminal not found. Turn it on, plus Location and Bluetooth. Keep WisePad 3 / M2 close. An S700 must be online in Stripe Dashboard → Terminal.',
  'Включите Геолокацию и Bluetooth. На WisePad 3 подержите кнопку питания, пока не замигает индикатор, затем нажмите «Найти терминал».':
      'Turn on Location and Bluetooth. On WisePad 3 hold power until the light blinks, then tap Find terminal.',
  'Не сопрягайте WisePad 3 в настройках Android Bluetooth — из‑за этого приложение его не видит. Забудьте устройство там, если уже сопрягали.\n\nВключите Геолокацию (точную) и Bluetooth. На WisePad 3 подержите питание, пока не замигает, держите рядом и нажмите «Найти терминал». Если появится код — подтвердите на телефоне и на ридере.':
      'Do not pair WisePad 3 in Android Bluetooth settings — then the app cannot see it. Forget it there if you already paired.\n\nTurn on precise Location and Bluetooth. Hold power on the WisePad until it blinks, keep it close, then tap Find terminal. If a code appears, confirm it on the phone and on the reader.',
  'Ищу WisePad 3. Держите его рядом, индикатор должен мигать. Не сопрягайте его в настройках Android.':
      'Looking for WisePad 3. Keep it close; the light should blink. Do not pair it in Android settings.',
  'WisePad 3 не найден. 1) Забудьте его в Настройки Android → Bluetooth. 2) Не сопрягайте там — только через это приложение. 3) Геолокация точная + Nearby devices. 4) Подержите кнопку питания на ридере, пока не замигает, и держите рядом.':
      'WisePad 3 not found. 1) Forget it in Android Settings → Bluetooth. 2) Do not pair it there — only through this app. 3) Precise Location + Nearby devices. 4) Hold power until it blinks and keep it close.',
  'Нашёл': 'Found',
  'Подключаю. Если появится 6‑значный код — подтвердите на телефоне и на WisePad.':
      'Connecting. If a 6-digit code appears, confirm it on the phone and on the WisePad.',
  'Разрешите «Рядом» / Nearby devices для этого приложения, иначе WisePad 3 не виден.':
      'Allow Nearby devices for this app, otherwise WisePad 3 will not appear.',
  'Включите Bluetooth в шторке телефона и нажмите «Найти терминал» ещё раз.':
      'Turn on Bluetooth in the Android shade and tap Find terminal again.',
  'Включите Геолокацию. Без неё WisePad 3 не находится.':
      'Turn on Location. Without it WisePad 3 cannot be found.',
  'Связь с ридером оборвалась. Забудьте WisePad в настройках Android Bluetooth и подключайте только через приложение.':
      'The reader connection dropped. Forget WisePad in Android Bluetooth settings and pair only through the app.',
  'Готовлю подключение...': 'Preparing the connection...',
  'Вид уведомлений': 'Notification look',
  'Цвет, размер, скрытие': 'Color, size, auto-hide',
  'Цвет плашки': 'Banner color',
  'Так выглядит баннер на телефоне. Звук по-прежнему в настройках Android.':
      'This is how the phone banner looks. Sound is still in Android settings.',
  'Цветная плашка': 'Colored banner',
  'Android красит фон уведомления': 'Android tints the notification background',
  'Крупный текст': 'Large text',
  'Исчезать через': 'Hide after',
  'Не скрывать': 'Keep on screen',
  'сек': 'sec',
  'Или смахните баннер пальцем. Входящий звонок не прячем.':
      'Or swipe the banner away. Incoming calls stay until you answer.',
  'Уведомление остаётся в шторке, пока не смахнёте. Входящий звонок не прячем.':
      'The notification stays in the shade until you swipe it away. Incoming calls stay.',
  'Экран блокировки': 'Lock screen',
  'На экране блокировки без текста': 'On the lock screen without text',
  'Не показывать на блокировке': 'Hide on the lock screen',
  'Показывать полностью': 'Show full text',
  'Показать пример': 'Show a preview',
  'Так выглядит уведомление. Можно смахнуть.':
      'This is how a notification looks. You can swipe it away.',
  'Меню': 'Menu',
  'Больше всего': 'Busiest',
  'Меньше всего': 'Quietest',
  'Нет данных за этот период': 'No data in this period',
  'Тихие часы': 'Quiet hours',
  'Тихие дни': 'Quiet days',
  'По часам': 'By hour',
  'По дням': 'By day',
  'Какие плитки показывать в боковом меню. Меню открывается свайпом вправо по нижней панели. Поиск — синяя плитка внизу справа.':
      'Choose which tiles appear in the side menu. Open the menu by swiping right on the bottom bar. Search is the blue tile at the bottom right.',
  'Часы в календаре, SMS и заявках: телефон или пояс по GPS':
      'Clock for calendar, SMS and jobs: phone time or GPS time zone',
  'Не даёт ставить визит впритык к другому и сдвигает время на маршруте':
      'Stops visits from overlapping and shifts times on the route',
  'Samsung может не показывать push, если приложение «усыплено». Нажмите — откроются настройки Android.':
      'Samsung may hide push if the app is sleeping. Tap to open Android settings.',
  'Уже без ограничений. Если push всё равно нет — в списке выберите FIX APPLIANCE.':
      'Already unrestricted. If push still fails, pick FIX APPLIANCE in the list.',
  'В Android выберите FIX APPLIANCE и «Не оптимизировать».':
      'In Android pick FIX APPLIANCE and Don’t optimize.',
  'Печатается на PDF счёта': 'Printed on the invoice PDF',
  'Попросить отзыв?': 'Ask for a review?',
  'Клиент оплатил счёт. Отправить SMS со ссылкой на Google-отзыв?':
      'The client paid the invoice. Send an SMS with the Google review link?',
  'Позже': 'Later',
  'Новое мероприятие': 'New event',
  'Мероприятие': 'Event',
  'Напишите текст мероприятия': 'Enter the event text',
  'Удалить мероприятие?': 'Delete this event?',
  'Добавить в календарь': 'Add to calendar',
  'Текст, время и фото': 'Text, time and photo',
};
